    ...
    setTimeout(() => {
        try {
            if (typeof atualizarInfoProgressoSyncCronos === 'function')
                atualizarInfoProgressoSyncCronos();
        }
        catch (e) { }
        try {
            if (typeof atualizarResumoSincronizacaoCronos === 'function')
                atualizarResumoSincronizacaoCronos();
        }
        catch (e) { }
    }, 0);
}
...
function inferirUrlSerieLancamentoCronos(nomeObra, urlEpisodio = '') {
    ...
    if (slugNome)
        return `${CRONOS_BASE_URL}series/${slugNome}/`;
    try {
...
function extrairLancamentoDeItemEpisodioCronos(item) {
    if (!item || !item.querySelector)
        return null;
    if (item.closest && item.closest('#slider-movies, #slider-tvshows, .slider, .featured, .sidebar, aside, .widget'))
        return null;
    ...
    if (!urlEpisodio || !urlEpisodio.includes('/episodios/'))
        return null;
    ...
    if (!key || !nomeObra || /^epis[oó]dio\s*\d+$/i.test(nomeObra))
        return null;
...
async function coletarLancamentosPaginaEpisodiosCronos(pagina = 1) {
    ...
    if (!res.ok)
        throw new Error('Falha ao ler episódios');
...
async function carregarLancamentosDeEpisodiosCronos(forcarCompleto = false) {
    ...
    try {
        ...
        const mapa = new Map();
        coletados.forEach(item => { if (item && item.key && !mapa.has(item.key))
            mapa.set(item.key, item); });
        if (!forcarCompleto && cache.length) {
            cache.forEach(item => { const key = item.key || chaveLancamentoCronos(item.titulo); if (key && !mapa.has(key))
                mapa.set(key, { ...item, key }); });
        }
...
function iniciarNavegacao(idTela, urlAlvo, btnElement) {
    ...
    if (idTela !== 'telaBusca' && idTela !== 'telaFavoritos' && idTela !== 'telaHistorico' && idTela !== 'telaCategorias' && idTela !== 'telaConfiguracoes' && idTela !== 'telaLancamentos')
        telaAnterior = idTela;
    ...
    if (idTela === 'telaConfiguracoes') {
        carregarConfiguracoes(btnElement);
        return;
    }
    ...
    if (grid.children.length > 0 && idTela !== 'telaBusca')
        return;
...
async function carregarHomePage() {
    ...
    try {
        ...
        atualizarDestaquePremium(0);
        let limitF = 0;
        filmesNormais.forEach(item => { if (limitF < 12) {
            renderizarItemNoGrid(item, 'gridInicioFilmes');
            limitF++;
        } });
        let limitS = 0;
        seriesNormais.forEach(item => { if (limitS < 12) {
            renderizarItemNoGrid(item, 'gridInicioSeries');
            limitS++;
        } });
...
function completarHomeGridAte12(urlAlvo, gridId, seletorItens, controlarTituloEpisodios = false) {
    const grid = document.getElementById(gridId);
    if (!grid || grid.children.length >= 12)
        return;
    fetch(PROXY + encodeURIComponent(urlAlvo))
        ...
        if (controlarTituloEpisodios) {
            const head = document.getElementById('headEpisodiosRecentes');
            if (head && grid.children.length > 0)
...
function obterBotaoMaisCronos(idTela) {
    if (idTela === 'telaBusca')
        return document.getElementById('btnMaisBusca');
...
async function carregarMais(idTela, urlBase) {
    ...
    for (let i = 0; i < CRONOS_PAGINAS_POR_CARREGAR_MAIS; i++) {
        ...
        if (!qtd)
            break;
        if (i < CRONOS_PAGINAS_POR_CARREGAR_MAIS - 1)
            await esperar(80);
    }
    if (btn)
        btn.disabled = false;
...
function obterSeletorItensCronos(idTela) {
    ...
    if (idTela === 'telaBusca')
        seletorItens = '#archive-content .item, .search-page .item, .result-item, article.item, .items .item, .module .content .items .item';
    if (idTela === 'telaEpisodios')
        seletorItens = '#archive-content .item, #dt-episodes .item, .episodes .item, article.episodes, article.episode';
    if (idTela === 'telaTemporadas')
        seletorItens = '#archive-content .item.seasons, #archive-content .item.se, article.seasons, article.item.se, article.item.seasons';
    return seletorItens;
}
...
function urlBaseDooplayPorTipo(tipo) {
    if (tipo === 'movies')
        return 'https://www.boraflix.click/filmes/';
    if (tipo === 'tvshows')
        return 'https://www.boraflix.click/series/';
...
function extrairStringsHtmlPossiveis(valor, saida = []) {
    if (typeof valor === 'string') {
        if (valor.includes('<article') || valor.includes('class="item') || valor.includes("class='item") || valor.includes('<li'))
            saida.push(valor);
        return saida;
    }
...
async function tentarGlossaryDooplayPorLetra(gridId, statusId, idTela, letra, tokenLocal) {
    ...
    for (const urlApi of urls) {
        if (tokenLocal !== tokenFiltroLetraCronos)
            return true;
        try {
            const res = await fetch(PROXY + encodeURIComponent(urlApi));
            const txt = await res.text();
            const html = htmlGlossaryDaResposta(txt);
            if (!html || /no_verify_nonce|No data nonce|rest_no_route/i.test(html))
                continue;
            const doc = new DOMParser().parseFromString(html, 'text/html');
...
async function renderizarResultadosLetraDoBancoCronos(gridId, letra, tipoGrid = '') {
    ...
    if (!grid)
        return 0;
    let store = 'obras';
    if (tipoGrid === 'episodio')
        store = 'episodios';
    if (tipoGrid === 'temporada')
        store = 'temporadas';
    const itensBanco = await dbGetAll(store);
    let total = 0;
    itensBanco.forEach(item => {
        const titulo = item.titulo || item.serieTitulo || '';
        if (!tituloPassaFiltroLetraCronos(titulo, letra))
            return;
        if (tipoGrid === 'filme' && item.tipo !== 'Filme')
            return;
        if (tipoGrid === 'serie' && item.tipo !== 'Série')
            return;
        if (grid.querySelector(`[data-url="${CSS.escape(item.url || '')}"]`))
            return;
...
async function montarCategorias() {
    ...
    if (!gridOficiais || !gridAchados || !gridAnos)
        return;
    await migrarLocalStorageParaIndexedDB();
    ...
    generosBanco.forEach(g => {
        ...
        if (oficiaisMap.has(g.slug))
            return;
        if (vistosAchados.has(g.slug))
            return;
    ...
    if (!gridAchados.children.length)
        gridAchados.innerHTML = '<div class="categoria-vazio">Nenhum gênero novo achado ainda. Use a sincronização para descobrir gêneros reais nas fichas.</div>';
    const setAnos = new Set();
    anosBanco.forEach(a => { if (a && a.ano)
        setAnos.add(String(a.ano)); });
    gridAnos.innerHTML = '';
    ...
    if (!gridAnos.children.length)
        gridAnos.innerHTML = '<div class="categoria-vazio">Nenhum ano salvo ainda. A sincronização preenche esta parte com os anos encontrados.</div>';
...
async function renderizarFiltroDoBancoComLetraCronos(tipo, titulo, gridId, letra = 'ALL') {
    ...
    obras.forEach(obra => {
        ...
        if (!passa)
            return;
        if (!tituloPassaFiltroLetraCronos(obra.titulo || '', letra))
            return;
        if (grid.querySelector(`[data-url="${CSS.escape(obra.url || '')}"]`))
            return;
...
async function abrirFiltroCategoria(tipo, titulo, urlBase) {
    ...
    if (status)
        status.innerText = `Banco local: ${qtdBanco} item(ns). Atualizando pelo site...`;
...
async function filtrarPorLetraCronos(contexto, letra, btn) {
    ...
    if (contexto === 'filtro') {
        ...
        if (tipoAtual === 'year') {
            ...
            const qtdBanco = await renderizarFiltroDoBancoComLetraCronos('year', contextoBuscaAtual.titulo || '', gridId, letra);
            if (status) {
                status.innerText = qtdBanco > 0
        ...
        if (tipoAtual === 'genre') {
            ...
            const qtdBanco = await renderizarFiltroDoBancoComLetraCronos('genre', contextoBuscaAtual.titulo || '', gridId, letra);
            if (status) {
                status.innerText = qtdBanco > 0
        ...
        const qtdBanco = await renderizarFiltroDoBancoComLetraCronos(tipoAtual, (contextoBuscaAtual && contextoBuscaAtual.titulo) || termoBuscaAtual || '', gridId, letra);
        if (status) {
            status.innerText = qtdBanco > 0
    ...
    if (contexto === 'series') {
        idTela = 'telaSeries';
        gridId = 'gridSeries';
        statusId = 'statusSeries';
        btnMaisId = 'btnMaisSeries';
        catalogoBase = 'https://www.boraflix.click/series/';
        tipoGrid = 'serie';
    }
    if (contexto === 'episodios') {
        idTela = 'telaEpisodios';
        gridId = 'gridEpisodios';
        statusId = 'statusEpisodios';
        btnMaisId = 'btnMaisEpisodios';
        catalogoBase = 'https://www.boraflix.click/episodios/';
        tipoGrid = 'episodio';
    }
    if (contexto === 'temporadas') {
        idTela = 'telaTemporadas';
        gridId = 'gridTemporadas';
        statusId = 'statusTemporadas';
        btnMaisId = 'btnMaisTemporadas';
        catalogoBase = 'https://www.boraflix.click/temporadas/';
        tipoGrid = 'temporada';
    }
...
async function buscarPosterDetalhePorUrl(urlObra) {
    if (!urlObra)
        return '';
    if (cachePosterDetalhe.has(urlObra))
        return cachePosterDetalhe.get(urlObra);
    try {
...
async function enriquecerPosterDoCardSeNecessario(li, itemOriginal, urlObra, imgAtual) {
    if (!li || !urlObra)
        return;
    if (itemJaTemPosterBom(itemOriginal) && !imagemPareceBackdropOuImagemFraca(imgAtual))
        return;
    if (!imagemPareceBackdropOuImagemFraca(imgAtual) && /\/w500\//i.test(String(imgAtual || '')))
        return;
...
function extrairDadosBasicosItem(item) {
    ...
    if (img && img.startsWith('//'))
        img = 'https:' + img;
    if (img)
        img = img.replace('w92', 'w500').replace('w185', 'w500');
    return {
        url: urlString,
        titulo,
        img,
    ...
    if (enriquecer && dados.url) {
        try {
            ...
            try {
                ...
                console.warn('Falha ao salvar destaque no banco:', dados.url, e2);
            }
        }
    }
    else {
        try {
            ...
            console.warn('Falha ao salvar destaque no banco:', dados.url, e);
        }
    }
    dados.providerKey = providerKeyAtual;
    dados.providerName = providerInfoAtual.nome;
...
function atualizarDestaquePremium(novoIndex) {
    ...
    destaquesPremiumHome.forEach((obra, index) => {
        ...
        if (obra.providerKey) {
            ...
            slide.dataset.providerSigla = obra.providerSigla || '';
        }
...
function extrairNomeObraDoEpisodio(item, tituloLimpoCard, urlString = '') {
    ...
    for (const sel of possiveis) {
        ...
        txt = txt
            ...
            .replace(/\s+/g, ' ')
            .trim();
        if (txt && txt.length > 3 && !/^epis[oó]dio\s*\d+$/i.test(txt))
            return txt;
    }
...
function detectarTipoPorUrl(url) {
    if (String(url || '').includes('/filmes/'))
        return 'Filme';
    if (String(url || '').includes('/episodios/') || String(url || '').includes('/episodio/'))
        return 'Episódio';
    if (String(url || '').includes('/temporadas/'))
        return 'Temporada';
    return 'Série';
}
function extrairGenerosDetalhe(doc) {
    ...
    doc.querySelectorAll('.genres .mta a, .sgeneros a, a[href*="/categoria/"]').forEach(a => {
        ...
        let slug = '';
        try {
            slug = new URL(url).pathname.split('/').filter(Boolean).pop() || slugCronos(nome);
        }
        catch (e) {
            slug = slugCronos(nome);
        }
        if (!generos.some(g => g.slug === slug))
            generos.push({ nome, slug, url: url || `${CRONOS_BASE_URL}categoria/${slug}/` });
    });
    return generos;
}
async function salvarGeneroCronos(genero) {
    if (!genero || !genero.slug)
        return;
    await dbPut('generos', {
        id: genero.slug,
        providerKey: CRONOS_PROVIDER_KEY,
...
async function salvarObraCronos(obra) {
    ...
    for (const genero of (registro.generos || []))
        await salvarGeneroCronos(genero);
    return registro;
}
...
function escolherPosterSeguroCronos(...urls) {
    for (const url of urls) {
        ...
        if (posterBomCronos(normalizada))
            return normalizada;
    }
    return '';
}
function escolherBackdropSeguroCronos(...urls) {
    for (const url of urls) {
        ...
        if (normalizada && !imagemEhPlaceholderCronos(normalizada))
            return normalizada;
    }
    return '';
}
...
function renderizarTemporadaCronos(item, gridId) {
    ...
    if (!dadosTemp || !dadosTemp.url)
        return;
    const letraAtivaGrid = filtroLetraGridAtual[gridId] || 'ALL';
    if (!tituloPassaFiltroLetraCronos(dadosTemp.serieTitulo || dadosTemp.titulo, letraAtivaGrid))
        return;
    if (gridDestino.querySelector(`[data-url="${CSS.escape(dadosTemp.url)}"]`))
        return;
    const li = document.createElement('li');
    li.className = 'card-item global-card';
...
function renderizarItemNoGrid(item, gridId) {
    ...
    if (!gridDestino)
        return;
    if (gridDestino.querySelector(`[data-url="${CSS.escape(urlString)}"]`))
        return;
    ...
    if (tipoForcadoGrid === 'filme' && !isMovie)
        return;
    if (tipoForcadoGrid === 'serie' && !isSerie)
        return;
    if (tipoForcadoGrid === 'episodio' && !isEpisodioHome)
        return;
    if (tipoForcadoGrid === 'temporada' && !isTemporada)
        return;
    const tituloLimpoCard = limparTextoCard(titleEl.innerText);
    const letraAtivaGrid = filtroLetraGridAtual[gridId] || 'ALL';
    if (!tituloPassaFiltroLetraCronos(tituloLimpoCard, letraAtivaGrid))
        return;
    let badgeHTML = '';
    if (isEpisodioHome)
        badgeHTML = `<div class="badge-tipo badge-serie">Episódio</div>`;
    else if (isMovie)
        badgeHTML = `<div class="badge-tipo badge-filme">Filme</div>`;
    else if (isSerie)
        badgeHTML = `<div class="badge-tipo badge-serie">Série</div>`;
...
function addLogSyncCronos(msg) {
    ...
    if (!log)
        return;
    if (log.children.length === 1 && log.innerText.includes('Aguardando'))
        log.innerHTML = '';
...
function toggleRotaSyncCronos(rota) {
    if (syncCronosAtiva)
        return;
    if (!Object.prototype.hasOwnProperty.call(syncCronosRotasAtivas, rota))
        return;
...
function atualizarControlesSyncCronos() {
    ...
    if (btnContinuar)
        btnContinuar.classList.toggle('ativo', syncCronosOperacao === 'continuar');
    if (btnReiniciar)
        btnReiniciar.classList.toggle('ativo', syncCronosOperacao === 'reiniciar');
    const mapa = {
    ...
    Object.entries(mapa).forEach(([rota, id]) => {
        ...
        if (btn)
            btn.classList.toggle('ativo', !!syncCronosRotasAtivas[rota]);
    });
}
...
async function iniciarOuPararSyncCronos(modo = 'completa') {
    if (syncCronosAtiva) {
        ...
        if (syncCronosController)
            syncCronosController.abort();
...
async function limparDadosSincronizadosCronos() {
    if (!confirm('Limpar dados sincronizados? Favoritos e histórico serão preservados.'))
        return;
...
function extrairItensCatalogoParaSync(doc, tipo) {
    ...
    if (!itens.length)
        itens = Array.from(doc.querySelectorAll('#archive-content .item, #archive-content article.item'));
    return itens.filter(item => {
        if (!item || !item.querySelector)
            return false;
        if (item.closest('#slider-movies, #slider-tvshows, .slider, .featured, .sidebar, aside, .widget'))
            return false;
        const url = item.querySelector('a')?.href || '';
        if (tipo === 'Filme')
            return url.includes('/filmes/') || item.classList.contains('movies');
        if (tipo === 'Série')
            return url.includes('/series/') || item.classList.contains('tvshows');
        if (tipo === 'Episódio')
            return url.includes('/episodios/');
        if (tipo === 'Temporada')
            return url.includes('/temporadas/') || item.classList.contains('seasons');
        return false;
    });
}
async function extrairDetalhesParaSyncCronos(url, tipo, dadosBase = {}) {
    ...
    if (!res.ok)
        throw new Error('Falha ao abrir detalhe');
    const html = await res.text();
    const doc = new DOMParser().parseFromString(html, 'text/html');
    ...
    if (tipo === 'Filme' && !playerUrl) {
        ...
        if (altPlayer && altPlayer.playerUrl)
            playerUrl = altPlayer.playerUrl;
    }
    return {
        ...dadosBase,
        url,
...
async function salvarEpisodiosDaSerieSyncCronos(doc, serie) {
    ...
    for (let sIndex = 0; sIndex < seasonsValidas.length; sIndex++) {
        ...
        for (let eIndex = 0; eIndex < eps.length; eIndex++) {
            ...
            if (epDate && epTitulo.includes(epDate))
                epTitulo = epTitulo.replace(epDate, '').trim();
            ...
            if (epImg && epImg.startsWith('//'))
                epImg = 'https:' + epImg;
            if (epImg && epImg.startsWith('/'))
                epImg = CRONOS_BASE_URL.replace(/\/$/, '') + epImg;
        ...
        for (let rIndex = indiceInicial; rIndex < rotas.length; rIndex++) {
            ...
            while (!syncCronosAbort) {
                ...
                try {
                    ...
                    if (!res.ok)
                        throw new Error('Fim ou falha de página');
                    html = await res.text();
                }
                catch (e) {
                    if (syncCronosAbort)
                        ...
                        try {
                            if (rota.tipo === 'Filme' || rota.tipo === 'Série') {
                                ...
                                if (rota.tipo === 'Série' && docDetalhe && salvo) {
                                    ...
                                    if (infoEps.episodios)
                                        addLogSyncCronos(`${salvo.titulo}: ${infoEps.temporadas} temporada(s), ${infoEps.episodios} episódio(s).`);
                                }
                            }
                            else {
...
function tipoObraCronos(obra) {
    ...
    if (obra?.isMovie)
        return 'Filme';
    if (obra?.temporada || obra?.episodio || String(obra?.url || '').includes('/episodios/'))
        return 'Episódio';
    return 'Série';
}
...
async function migrarLocalStorageParaIndexedDB() {
    ...
    try {
        ...
        for (const obra of favsAntigos) {
            if (obra?.url)
                await dbPut('favoritos', normalizarObraParaBanco(obra));
        }
        const histAntigo = JSON.parse(localStorage.getItem('cronos_boraflix_history') || '[]');
        for (const obra of histAntigo) {
            if (obra?.url)
                await dbPut('historico', normalizarObraParaBanco(obra));
        }
...
function adicionarBotaoRemoverHoverCronos(li, storeName, url) {
    if (!li || !storeName || !url)
        return;
    if (li.querySelector('.btn-remover-card'))
        return;
    const btn = document.createElement('button');
    ...
    btn.onclick = async (event) => {
        ...
        if (storeName === 'favoritos') {
            ...
            if (status && grid && !grid.children.length)
                status.style.display = 'block';
        }
...
function renderizarObraSalvaHome(obra, gridId) {
    ...
    grid.appendChild(li);
    if (gridId !== 'gridHomeFavoritos')
        adicionarBotaoFavoritarHoverCronos(li, { ...obra, img: posterHome, poster: posterHome });
}
async function renderizarResumoHomeLocal() {
    ...
    if (emptyFav)
        emptyFav.style.display = favs.length ? 'none' : 'block';
    if (emptyHist)
        emptyHist.style.display = hist.length ? 'none' : 'block';
    ...
    if (statFavs)
        statFavs.innerText = favs.length;
    if (statHist)
        statHist.innerText = hist.length;
    if (statFilmes)
        statFilmes.innerText = document.getElementById('gridInicioFilmes') ? document.getElementById('gridInicioFilmes').children.length : 0;
    if (statSeries)
        statSeries.innerText = document.getElementById('gridInicioSeries') ? document.getElementById('gridInicioSeries').children.length : 0;
...
function extrairTextoCampoCustomDetalhe(doc, nomes = []) {
    ...
    for (const el of blocos) {
        ...
        if (!termos.some(t => upper.includes(t)))
            continue;
...
function extrairQualidadeDetalheCronos(doc, html = '') {
    ...
    if (qNode && qNode.innerText.trim())
        return qNode.innerText.trim().toUpperCase();
    if (texto.includes('4K') || texto.includes('2160P'))
        return '4K';
    if (texto.includes('1080P') || texto.includes('FHD') || texto.includes(' FULL HD') || texto.includes(' HD'))
        return 'HD';
    if (texto.includes('720P'))
        return 'HD';
    if (texto.includes('480P') || texto.includes(' SD'))
        return 'SD';
    return 'HD';
}
...
function extrairDataDetalheCronos(doc, anoFallback = '') {
    ...
    if (!val)
        val = extrairTextoCampoCustomDetalhe(doc, ['DATA DE LANÇAMENTO', 'PRIMEIRA DATA DE EXIBIÇÃO', 'FIRST AIR DATE', 'LANÇAMENTO', 'EXIBIÇÃO']);
...
function extrairLinkPlayerDetalheCronos(doc) {
    ...
    iframes.forEach((iframe) => {
        ...
        if (!src || src.includes('/ads/') || src.includes('22bet'))
            return;
        if (src.startsWith('//'))
            src = 'https:' + src;
        if (src.includes('superflixapi') || src.includes('superembeds') || src.includes('embed'))
            linkPlayer = src;
        else if (!linkPlayer)
            linkPlayer = src;
    });
    ...
    doc.querySelectorAll('meta[property="og:image"]').forEach(meta => {
        ...
        for (let i = 0; el && i < 5; i++, el = el.nextElementSibling) {
            ...
            if (prop === 'og:image:width')
                width = parseInt(el.getAttribute('content') || '0', 10) || 0;
            if (prop === 'og:image:height')
                height = parseInt(el.getAttribute('content') || '0', 10) || 0;
        }
        ...
        try {
            ...
            for (const obj of pilha) {
                ...
                if (typeof thumb === 'string' && thumb)
                    return limparUrlMidiaCronos(thumb);
            }
        }
        catch (e) { }
    }
...
function extrairPosterClickDetalhe(doc, fallback = '') {
    const candidatos = [];
    const add = (url) => { url = removerSufixoWpImagemCronos(url); if (url)
        candidatos.push(url); };
    ...
    coletarOgImagesCronos(doc).forEach(o => {
        if (o.width && o.height && o.height > o.width)
            add(o.url);
    });
...
function extrairBackdropClickDetalhe(doc, html = '') {
    const candidatos = [];
    const add = (url) => { url = limparUrlMidiaCronos(url); if (url)
        candidatos.push(url); };
    ...
    if (bgMatch && bgMatch[1])
        add(bgMatch[1]);
    coletarOgImagesCronos(doc).forEach(o => {
        if ((o.width && o.height && o.width >= o.height) || urlPareceHorizontalCronos(o.url))
            add(o.url);
    });
...
function extrairCustomClick(doc, nomes = []) {
    ...
    for (const el of blocosCustom) {
        ...
        if (!termos.some(t => up.includes(t)))
            continue;
...
function extrairSinopseClick(doc) {
    ...
    if (!sinopseNode)
        return 'Sinopse não disponível nos registros.';
...
function extrairQualidadeClick(doc, html = '') {
    ...
    let qualidade = 'HD';
    if (texto.includes('4K'))
        qualidade = '4K';
    else if (texto.includes('SD ') || texto.includes(' 480P'))
        qualidade = 'SD';
    else if (doc.querySelector('.quality, .Qlty'))
        qualidade = doc.querySelector('.quality, .Qlty').innerText.trim();
    return qualidade || 'HD';
}
function extrairDuracaoClick(doc) {
    const runtimeNode = doc.querySelector('.runtime, .metadata span.runtime, .Info span.Time');
    if (runtimeNode && runtimeNode.innerText.trim())
        return runtimeNode.innerText.trim();
...
function extrairLinkLimpoDoPlayer(doc, html = '') {
        ...
        const resolver = (href) => {
            try {
                ...
                if (href.startsWith('//'))
                    href = 'https:' + href;
                if (href.startsWith('/'))
                    href = new URL(href, (window.obraSendoVista && window.obraSendoVista.baseUrl) || CRONOS_BASE_URL || location.href).href;
    ...
    if (playerSuaapCronos)
        return playerSuaapCronos;
    const candidatos = [];
    const vistos = new Set();
    ...
    const addCandidato = (src, origem = '') => {
        ...
        if (u.includes('/ads/') || u.includes('22bet') || u.includes('doubleclick') || u.includes('adserver') || u.includes('facebook'))
            return;
        ...
        if (u.includes('superflixapi'))
            peso = 100;
        else if (u.includes('superembeds') || u.includes('superembed'))
            peso = 90;
        else if (u.includes('viewplayer') || u.includes('playerthree') || u.includes('trembed'))
            peso = 80;
...
function cronosRotuloPlayer(player, idx = 0) {
    ...
    if (!label) {
        if (u.includes('megaembed'))
            label = 'MegaEmbed';
        else if (u.includes('superflixapi') || u.includes('superembed') || u.includes('superembeds'))
            label = 'Dublado [SEM ANÚNCIO]';
        else if (u.includes('viewplayer') || u.includes('playerthree') || u.includes('trembed'))
            label = 'Dublado';
        else if (u.includes('suaap'))
            label = 'Dublado';
        else
            label = 'Player';
    }
    if (/legendado|\bleg\b/i.test(label))
        return 'Legendado';
    if (/sem\s*an[úu]ncio|superflix|superembed|megaembed/i.test(label))
        return label.replace(/superflixapi/ig, 'Dublado [SEM ANÚNCIO]');
    if (/dublado|\bdub\b/i.test(label))
        return 'Dublado';
...
function cronosPushPlayer(lista, vistos, src, label = '', tipo = 'direto', base = '') {
    ...
    if (!src || cronosPlayerBloqueado(src))
        return;
    ...
    try {
        if (doc && doc.querySelectorAll) {
            doc.querySelectorAll('a[href*="suaap.com/api/start/cpurl"], a[href*="api/start/cpurl"]').forEach(a => {
                ...
                if (real)
                    cronosPushPlayer(lista, vistos, real, label, 'suaap');
            });
        }
    }
    catch (e) { }
    try {
        ...
        for (const reg of regs)
            while ((m = reg.exec(texto))) {
                ...
                if (real)
                    cronosPushPlayer(lista, vistos, real, 'Dublado', 'suaap-regex');
            }
    }
    catch (e) { }
}
function cronosExtrairPlayersDetalhe(doc, html, iframePrincipal = '') {
        ...
        if (doc && doc.querySelectorAll) {
            ...
            doc.querySelectorAll('[id^="source-player-"]').forEach(box => {
                ...
                box.querySelectorAll('[data-source], [data-src], a[href]').forEach(el => {
                    ...
                    if (/player|embed|suaap|superflix|viewplayer|trembed|mega/i.test(src))
                        cronosPushPlayer(lista, vistos, src, label || el.innerText || '', 'source-data', location.href);
                });
            });
            const seletores = [
    ...
    try {
        ...
        const regexSourceBox = /<div[^>]+id=["']source-player-[^"']+["'][^>]*>[\s\S]*?<iframe[^>]+src=["']([^"']+)["'][^>]*>/gi;
        while ((m = regexSourceBox.exec(texto)))
            cronosPushPlayer(lista, vistos, m[1], '', 'regex-source');
        const regexIframe = /<iframe[^>]+src=["']([^"']+)["'][^>]*>/gi;
        while ((m = regexIframe.exec(texto)))
            cronosPushPlayer(lista, vistos, m[1], '', 'regex-iframe');
        const regexDireto = /(https?:\/\/[^"'<>\s]*(?:superflixapi|superembeds|superembed|megaembed|viewplayer|playerthree|trembed|suaap)[^"'<>\s]*)/gi;
        while ((m = regexDireto.exec(texto)))
            cronosPushPlayer(lista, vistos, m[1], '', 'regex-direto');
    }
    catch (e) { }
    if (iframePrincipal)
        cronosPushPlayer(lista, vistos, iframePrincipal, 'Player Original', 'principal');
    return lista;
}
async function cronosExpandirPlayerInterno(player) {
    ...
    if (!src || !podeExpandir || lower.includes('megaembed'))
        return [];
    try {
...
async function renderizarBotoesPlayerUnificadoCronos(titulo, doc, htmlDetalhe, iframeSrc) {
    ...
    final.forEach((p, idx) => {
        ...
        if (isOriginal)
            btn.style.cssText = 'background:#111;border:1px solid #555;color:#aaa;font-size:11px;padding:7px 10px;';
...
function analisarObra(url, ano, tituloCard, img, isMovie) {
    ...
    fetch(PROXY + encodeURIComponent(url))
        .then(res => { if (!res.ok)
        throw new Error('Falha HTTP'); return res.text(); })
        .then(async (html) => {
        ...
        const posterManualSalvoDetalhe = (obraSalvaAntesDetalhe?.posterManual || obraSalvaAntesDetalhe?.posterManualEditable || obraSalvaAntesDetalhe?.posterManualUpload)
            ...
            : '';
        if (posterManualSalvoDetalhe)
            posterSrc = posterManualSalvoDetalhe;
        ...
        if (dataFinal)
            linha2Html += `<span class="tag-meta">${dataFinal}</span>`;
        ...
        if (isMovie) {
            ...
            if (seasonsDiv.length > 0) {
                ...
                if (bEps)
                    bEps.innerText = '0 EPISÓDIOS';
                const bTem = document.getElementById('badgeTemporadas');
                if (bTem)
                    bTem.innerText = '0 TEMPORADAS';
            }
        }
    })
        .catch(err => {
...
function alternarAudioStreaming(audioSelecionado) {
    ...
    document.querySelectorAll('.btn-audio-tag').forEach(btn => {
        if (btn.getAttribute('data-audio') === audioSelecionado)
            btn.classList.add('ativo');
        else
            btn.classList.remove('ativo');
    });
    ...
    if (badgeTemporadas)
        badgeTemporadas.innerText = `${sids.length} TEMPORADA${sids.length !== 1 ? 'S' : ''}`;
    let totalEps = 0;
    ...
    if (badgeEps)
        badgeEps.innerText = `${totalEps} EPISÓDIO${totalEps !== 1 ? 'S' : ''}`;
    if (!sids.includes(temporadaAtiva))
        temporadaAtiva = sids[0] || null;
...
function renderizarGradeEpisodios() {
    ...
    if (sids.length === 0)
        return;
    if (!temporadaAtiva || !sids.includes(temporadaAtiva))
        temporadaAtiva = sids[0];
    if (sids.length > 1) {
    ...
    episDaTemporada.forEach((li, index) => {
        ...
        if (!epDate)
            epDate = obraSendoVista.ano || new Date().getFullYear();
        ...
        if (thumbUrl && thumbUrl.startsWith('/'))
            thumbUrl = CRONOS_BASE_URL.replace(/\/$/, '') + thumbUrl;
        if (!thumbUrl) {
        ...
        `;
        if (nativeUrl)
            epCard.onclick = () => prepararEpisodioDooplay(`${tituloSerieAtual} - ${tituloFormatado}`, nativeUrl);
        else if (iframeId)
            epCard.onclick = () => {
                const urlPlayerEpCronos = `https://viewplayer.online/episodio/${iframeId}`;
                if (window.cronosAbrirModalFontesEpisodioOficial)
                    window.cronosAbrirModalFontesEpisodioOficial(`${tituloSerieAtual} - ${tituloFormatado}`, urlPlayerEpCronos, () => abrirPlayer(`${tituloSerieAtual} - ${tituloFormatado}`, urlPlayerEpCronos));
                else
                    abrirPlayer(`${tituloSerieAtual} - ${tituloFormatado}`, urlPlayerEpCronos);
            };
...
function extrairEpisodiosNativos(doc, tituloSerie, posterSerie = '', backdropSerie = '', anoSerie = '') {
    ...
    if (totalEps === 0) {
        ...
        const badgeEps = document.getElementById('badgeEpisodios');
        if (badgeEps)
            badgeEps.innerText = '0 EPISÓDIOS';
        const badgeTem = document.getElementById('badgeTemporadas');
        if (badgeTem)
            badgeTem.innerText = '0 TEMPORADAS';
        return;
    }
...
function extrairEpisodiosIframeMestre(src, tituloSerie, posterSerie = '', backdropSerie = '', anoSerie = '') {
    ...
    fetch(PROXY + encodeURIComponent(src))
        .then(res => { if (!res.ok)
        throw new Error(); return res.text(); })
        .then(html => {
        msgEps.remove();
        const doc = new DOMParser().parseFromString(html, 'text/html');
        ...
        if (!cards.length) {
            ...
            if (badgeEpisodios)
                badgeEpisodios.innerText = `Catálogo Principal Unificado`;
            return;
        }
        ...
        cards.forEach(card => {
            ...
            eps.forEach(li => {
                ...
                if (!dadosSeriesAtual[audioKey][sid])
                    dadosSeriesAtual[audioKey][sid] = [];
                dadosSeriesAtual[audioKey][sid].push(li);
            });
        });
        localAudioAtivo = temDublado ? 'dublado' : 'legendado';
        if (!temDublado && !temLegendado) {
            temDublado = true;
            localAudioAtivo = 'dublado';
        }
        ...
        if (tagQualidade) {
            if (temLegendado) {
                ...
                btnLeg.innerText = 'Legendado';
                btnLeg.setAttribute('data-audio', 'legendado');
...
function prepararEpisodioDooplay(tituloEpisodio, urlEpisodio) {
    ...
    fetch(PROXY + encodeURIComponent(urlEpisodio))
        ...
        if (linkPlayer)
            abrirPlayer(tituloEpisodio, linkPlayer);
        else
...
function abrirPlayer(titulo, urlVideo) {
    if (obraSendoVista && obraSendoVista.url)
        salvarHistoricoHome({ ...obraSendoVista, titulo: obraSendoVista.titulo || titulo, playerUrl: urlVideo });
