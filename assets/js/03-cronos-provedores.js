(function () {
    ...
    const log = (...a) => { if (DEBUG)
        console.log('[CRONOS V22 NAV REAL]', ...a); };
    ...
    function epNums(txt) {
        ...
        if (m)
            return { temp: pad2(m[1]), ep: pad2(m[2]) };
        m = t.match(/(?:epis[oó]dio|episode|ep\.?|e)\s*(\d{1,4})/i);
        if (m)
            return { temp: '', ep: pad2(m[1]) };
        return { temp: '', ep: '' };
    }
    function ehFilme() {
        try {
            ...
            if (o.isMovie === true)
                return true;
            if (String(o.tipo || '').toLowerCase() === 'filme')
                return true;
            if (String(o.url || '').includes('/filmes/'))
                return true;
            if (String(o.url || '').includes('/filme/'))
                return true;
        }
        catch (e) { }
        return false;
    ...
    function serieAtual() {
        try {
            if (typeof tituloSerieAtual !== 'undefined' && tituloSerieAtual)
                return clean(tituloSerieAtual);
        }
        catch (e) { }
        try {
            if (typeof obraSendoVista !== 'undefined' && obraSendoVista && obraSendoVista.titulo)
                return clean(obraSendoVista.titulo);
        }
        catch (e) { }
        return 'Série';
    }
    function mapaAtual() {
        try {
            if (typeof dadosSeriesAtual === 'undefined' || !dadosSeriesAtual)
                return null;
    ...
    function episodioUtil(li) {
        ...
        if (txt.includes('aguardando lançamento') || txt.includes('ainda não foi lançado') || txt.includes('em breve'))
            return false;
    ...
    function listaReal() {
        ...
        let sid = sids[0];
        try {
            if (typeof temporadaAtiva !== 'undefined' && temporadaAtiva && mapa[temporadaAtiva])
                sid = temporadaAtiva;
        }
        catch (e) { }
    ...
    function garantirNav() {
        ...
        if (document.getElementById('cronosPlayerBottomRowV24')) {
            const rowV24 = document.getElementById('cronosPlayerBottomRowV24');
            if (nav.parentElement !== rowV24)
                rowV24.appendChild(nav);
        }
        else if (nav.previousElementSibling !== wrap)
    ...
    function esconder() {
        ...
        ['btnEpAnterior', 'btnEpProximo'].forEach(id => {
            ...
            if (b) {
                b.classList.remove('cronos-v22-show');
                b.style.display = '';
                b.onclick = null;
            }
        });
    ...
    function acharIndice(titulo, urlVideo) {
        ...
        if (pend && Number.isInteger(pend.index) && lista[pend.index])
            return pend.index;
    ...
    function abrirEntrada(ep) {
        ...
        if (ep.nativeUrl && typeof window.prepararEpisodioDooplay === 'function')
            window.prepararEpisodioDooplay(ep.titulo, ep.nativeUrl);
        else if (typeof window.abrirPlayer === 'function')
            window.abrirPlayer(ep.titulo, ep.playerUrl || ep.url);
    }
    ...
    if (typeof prepBase === 'function' && !prepBase.__cronosV22Real) {
        window.prepararEpisodioDooplay = function (tituloEpisodio, urlEpisodio) {
            ...
            let idx = -1;
            if (nums.ep)
                idx = lista.findIndex(x => x.ep === nums.ep && (!nums.temp || x.temp === nums.temp));
            if (idx < 0) {
...
(function () {
    ...
    function detectarNomePlayerCronosV24(url) {
        ...
        if (/superflixapi/i.test(u))
            return 'Superflix API';
        if (/superembeds/i.test(u))
            return 'Superembeds';
        if (/superembed/i.test(u))
            return 'Superembed';
        if (/viewplayer/i.test(u))
            return 'ViewPlayer';
        if (/playerthree/i.test(u))
            return 'PlayerThree';
        if (/trembed/i.test(u))
            return 'Trembed';
        return 'Player';
    ...
    function garantirEstruturaPlayerLabelV24() {
        ...
        if (labelMobile.parentElement !== controles)
            controles.insertBefore(labelMobile, fechar);
        if (fechar.parentElement !== controles)
            controles.appendChild(fechar);
        let row = document.getElementById('cronosPlayerBottomRowV24');
        if (!row) {
            row = document.createElement('div');
            row.id = 'cronosPlayerBottomRowV24';
        }
        if (row.previousElementSibling !== wrapper)
        ...
        if (labelDesktop.parentElement !== row)
            row.insertBefore(labelDesktop, row.firstChild);
        if (nav && nav.parentElement !== row)
            row.appendChild(nav);
    }
    function atualizarNomePlayerCronosV24(url) {
        ...
        if (desktop)
            desktop.textContent = nome;
        if (mobile)
            mobile.textContent = nome;
    }
    const abrirBase = window.abrirPlayer;
...
(function () {
    ...
    const mo = new MutationObserver(() => {
        const tela = document.getElementById('telaPlayer');
        if (tela && tela.classList.contains('ativa'))
            requestAnimationFrame(travarLinhaPlayerV26);
    });
    function iniciar() {
        travarLinhaPlayerV26();
        const tela = document.getElementById('telaPlayer');
        if (tela)
            mo.observe(tela, { childList: true, subtree: true });
    }
    if (document.readyState === 'loading')
        document.addEventListener('DOMContentLoaded', iniciar);
    else
...
(function () {
    ...
    function bancoPrimeiroAtivo() {
        try {
            return localStorage.getItem(PRIORIDADE_KEY) !== '0';
        }
        catch (e) {
            return true;
        }
    ...
    function setBancoPrimeiroAtivo(ativo) {
        try {
            localStorage.setItem(PRIORIDADE_KEY, ativo ? '1' : '0');
        }
        catch (e) { }
    ...
    function garantirPainelBancoV27() {
        ...
        if (syncTitulo)
            tela.insertBefore(painel, syncTitulo);
        else {
            const atalhos = tela.querySelector('.categoria-atalhos');
            if (atalhos && atalhos.nextSibling)
                tela.insertBefore(painel, atalhos.nextSibling);
            else
                tela.insertBefore(painel, tela.firstChild);
        }
        ...
        if (input) {
            input.addEventListener('change', e => {
                ...
                if (file)
                    cronosImportarBancoIndexedDB(file).finally(() => { input.value = ''; });
            });
        }
        atualizarPainelBancoV27();
    }
    ...
    window.cronosExportarBancoIndexedDB = async function () {
        ...
        try {
            if (status)
                status.textContent = 'Exportando banco IndexedDB...';
            if (typeof migrarLocalStorageParaIndexedDB === 'function')
            ...
            if (status)
                status.textContent = 'Banco exportado com sucesso.';
        }
        catch (e) {
            console.error('Falha ao exportar banco:', e);
            if (status)
                status.textContent = 'Erro ao exportar banco. Veja o console.';
    ...
    window.cronosImportarBancoIndexedDB = async function (file) {
        ...
        try {
            ...
            if (!ok)
                return;
            if (status)
                status.textContent = 'Importando banco IndexedDB...';
            if (typeof abrirCronosDB === 'function')
                await abrirCronosDB();
            for (const store of stores) {
                if (!payload.data[store] || !Array.isArray(payload.data[store]))
                    continue;
                try {
                    await dbClear(store);
                }
                catch (e) {
                    console.warn('Não limpou store', store, e);
                }
                for (const item of payload.data[store]) {
                    ...
                    try {
                        ...
                        console.warn('Falha item importado', store, item, e);
                    }
                }
            }
            ...
            if (typeof atualizarResumoSincronizacaoCronos === 'function')
                await atualizarResumoSincronizacaoCronos();
            if (typeof atualizarInfoProgressoSyncCronos === 'function')
                await atualizarInfoProgressoSyncCronos();
            if (status)
                status.textContent = 'Banco importado com sucesso. Reabra a aba desejada para recarregar os dados.';
            alert('Banco importado com sucesso.');
        }
        catch (e) {
            console.error('Falha ao importar banco:', e);
            if (status)
                status.textContent = 'Erro ao importar banco. Verifique se o arquivo é um backup válido.';
            alert('Erro ao importar banco. Verifique se o arquivo é um backup válido.');
        }
    };
    function gridUsaSitePrimeiroV27(gridId) {
        if (bancoPrimeiroAtivo())
            return false;
    ...
    window.prepararCardObraDBFirst = async function (li, dadosBasicos, itemOriginal, gridId) {
        ...
        try {
            ...
            try {
                preencherCardObraCronos(li, { ...dadosBasicosParaObra(dadosBasicos), poster: placeholderCronosPoster(), img: placeholderCronosPoster() }, gridId);
            }
            catch (_) { }
        }
    };
    ...
    if (document.readyState === 'loading')
        document.addEventListener('DOMContentLoaded', garantirPainelBancoV27);
    else
        setTimeout(garantirPainelBancoV27, 0);
})();
;
(function () {
    ...
    function providerKeyCronosV41() {
        try {
            if (typeof CRONOS_PROVIDER_KEY !== 'undefined' && CRONOS_PROVIDER_KEY)
                return String(CRONOS_PROVIDER_KEY);
        }
        catch (e) { }
        try {
            if (typeof SITE_CODE !== 'undefined' && SITE_CODE)
                return String(SITE_CODE);
        }
        catch (e) { }
        return 'CRONOS';
    }
    function baseUrlCronosV41() {
        try {
            if (typeof CRONOS_BASE_URL !== 'undefined' && CRONOS_BASE_URL)
                return String(CRONOS_BASE_URL);
        }
        catch (e) { }
    ...
    function urlCanonicaCronosV41(url) {
        ...
        if (!u)
            return '';
        try {
            u = new URL(u, baseUrlCronosV41()).href;
        }
        catch (e) { }
        return String(u)
            .replace(/[?#].*$/, '')
    ...
    async function listaUnicaStoreV41(store) {
        ...
        for (const item of todos) {
            ...
            if (precisaCorrigir) {
                if (item.id && item.id !== corrigido.id)
                    await deleteStoreV41(store, item.id);
    ...
    if (typeof normalizarOriginalV41 === 'function' && !normalizarOriginalV41.__cronosUnicoV41) {
        const normalizarCorrigida = function (obra = {}) {
            ...
            if (!registro.providerName) {
                try {
                    registro.providerName = CRONOS_PROVIDER_NAME;
                }
                catch (e) {
                    registro.providerName = 'Cronos';
                }
            }
            return registro;
        };
        ...
        try {
            normalizarObraParaBanco = normalizarCorrigida;
        }
        catch (e) { }
    }
    ...
    try {
        getFavoritos = window.getFavoritos;
    }
    catch (e) { }
    ...
    try {
        isFavoritoCronos = window.isFavoritoCronos;
    }
    catch (e) { }
    window.toggleFavorito = async function () {
        ...
        if (existentes.length) {
            for (const fav of existentes)
                await deleteStoreV41('favoritos', fav.id);
            if (btn) {
        ...
        if (document.getElementById('telaFavoritos')?.classList.contains('ativa') && typeof carregarFavoritos === 'function')
            carregarFavoritos();
    };
    try {
    ...
    try {
        checarBotaoFavorito = window.checarBotaoFavorito;
    }
    catch (e) { }
    window.adicionarBotaoFavoritarHoverCronos = function (li, dados) {
        if (!li || !dados || !dados.url)
            return;
        if (li.querySelector('.btn-favoritar-card'))
            return;
    ...
    try {
        adicionarBotaoFavoritarHoverCronos = window.adicionarBotaoFavoritarHoverCronos;
    }
    catch (e) { }
    ...
    if (typeof carregarFavOriginalV41 === 'function') {
        ...
        try {
            carregarFavoritos = window.carregarFavoritos;
        }
        catch (e) { }
    }
    async function salvarHistoricoUnicoV41(item) {
        if (!item || !item.url || !item.titulo || item.titulo === 'undefined')
            return;
        await garantirDBV41();
    ...
    try {
        salvarHistoricoHome = salvarHistoricoUnicoV41;
    }
    catch (e) { }
    ...
    try {
        getHistoricoHome = window.getHistoricoHome;
    }
    catch (e) { }
    ...
    if (typeof carregarHistOriginalV41 === 'function') {
        ...
        try {
            carregarHistorico = window.carregarHistorico;
        }
        catch (e) { }
    }
    ...
    function aplicarAjusteConfigLancamentos() {
        ...
        if (cat) {
            [...cat.querySelectorAll('.categoria-atalhos button,.categoria-atalhos a,.cronos-categoria-atalhos-letras button,.cronos-categoria-atalhos-letras a')].forEach(b => {
                ...
                if (t.includes('episod') || t.includes('lancamento') || t.includes('anime'))
                    b.remove();
            });
            [...cat.querySelectorAll('.categoria-atalhos,.cronos-categoria-atalhos-letras')].forEach(box => {
                if (!box.querySelector('button,a'))
                    box.remove();
            });
        }
        ...
        if (!box) {
            ...
            if (titulo && titulo.nextSibling)
                cfg.insertBefore(box, titulo.nextSibling);
            else
                cfg.insertBefore(box, cfg.firstChild);
        }
        ...
        let btnTemp = acharBotaoPorTexto(cfg, 'Temporadas') || criarBotaoAtalho('Temporadas', function (e) {
            ...
            if (typeof window.iniciarNavegacao === 'function')
                window.iniciarNavegacao('telaTemporadas', 'https://www.boraflix.click/temporadas/', document.querySelector('button[onclick*=\'carregarConfiguracoes\']') || this);
        });
        let btnEp = acharBotaoPorTexto(cfg, 'Episódios') || criarBotaoAtalho('Episódios', function (e) {
            e.preventDefault();
            if (typeof window.iniciarNavegacao === 'function')
                window.iniciarNavegacao('telaEpisodios', 'https://www.boraflix.click/episodios/', document.querySelector('button[onclick*=\'carregarConfiguracoes\']') || this);
        });
        ...
        let btnLanc = acharBotaoPorTexto(cfg, 'Lançamentos') || criarBotaoAtalho('Lançamentos', function (e) {
            ...
            if (typeof window.carregarLancamentos === 'function')
                window.carregarLancamentos(document.querySelector('button[onclick*=\'carregarConfiguracoes\']') || this);
            else if (typeof window.ativarTela === 'function')
                window.ativarTela('telaLancamentos', this);
        });
        let btnAnime = acharBotaoPorTexto(cfg, 'Animes') || acharBotaoPorTexto(cfg, 'Anime') || criarBotaoAtalho('Animes', function (e) {
            e.preventDefault();
            if (typeof window.iniciarNavegacao === 'function')
                window.iniciarNavegacao('telaAnimes', 'https://primeflix.mom/animes', document.querySelector('button[onclick*=\'carregarConfiguracoes\']') || this);
        });
...
;
(function () {
    if (window.__CRONOS_MULTI_PROVIDER_PATCH__)
        return;
    ...
    const ORDEM_PROVEDORES = ['b01', 'p02', 'm03', 'e04', 's05', 'u06', 'l07', 'primeflix'];
    const PROVIDERS = {
        b01: { key: 'b01', sigla: 'BFCK', nome: 'Fonte BFCK', base: 'https://www.boraflix.click/', generoBase: '/categoria/', semEpisodios: false, semAno: false },
        p02: { key: 'p02', sigla: 'BFTV', nome: 'Fonte BFTV', base: 'https://www.boraflixtv.com/', generoBase: '/categoria/', semEpisodios: false, semAno: false },
        m03: { key: 'm03', sigla: 'MEGA', nome: 'Fonte MEGA', base: 'https://megacine.boats/', generoBase: '/generos/', semEpisodios: true, semAno: true, semLetras: true },
        e04: { key: 'e04', sigla: 'EBA', nome: 'Fonte EBA', base: 'https://www.ebaflix.com/', generoBase: '', semCategoria: true, semEpisodios: true, semAno: true },
        s05: { key: 's05', sigla: 'SWEB', nome: 'Fonte SWEB', base: 'https://www.seriesonlineweb.lol/', generoBase: '/generos/', semEpisodios: true, semAno: true },
        u06: { key: 'u06', sigla: 'SSUP', nome: 'Fonte SSUP', base: 'https://superseries.life/', generoBase: '/generos/', semEpisodios: true, semAno: true },
        l07: { key: 'l07', sigla: 'LISO', nome: 'Fonte LISO', base: 'https://lisoflix.net/', generoBase: '/category/', filmePath: '/movies/', seriePath: '/series/', episodioPath: '/episodio/', buscaPath: '/?s=', letrasPath: '/letra/', semEpisodios: false, semAno: true },
        primeflix: { key: 'primeflix', sigla: 'PRIME', nome: 'Fonte PRIME', base: 'https://primeflix.mom/', generoBase: '', filmePath: '/filmes', seriePath: '/series', episodioPath: '/episodios', buscaPath: '/pesquisar?s=', animePath: '/animes', doramaPath: '/doramas', semCategoria: true, semAno: true, semLetras: true, semEpisodios: false }
    };
    const GENEROS_P02_MULTI = [{ "nome": "Ação", "slug": "acao", "url": "https://www.boraflixtv.com/categoria/acao/" }, { "nome": "Action & Adventure", "slug": "action-adventure", "url": "https://www.boraflixtv.com/categoria/action-adventure/" }, { "nome": "Animação", "slug": "animacao", "url": "https://www.boraflixtv.com/categoria/animacao/" }, { "nome": "Aventura", "slug": "aventura", "url": "https://www.boraflixtv.com/categoria/aventura/" }, { "nome": "Canais", "slug": "canais", "url": "https://www.boraflixtv.com/categoria/canais/" }, { "nome": "Cinema TV", "slug": "cinema-tv", "url": "https://www.boraflixtv.com/categoria/cinema-tv/" }, { "nome": "Comédia", "slug": "comedia", "url": "https://www.boraflixtv.com/categoria/comedia/" }, { "nome": "Crime", "slug": "crime", "url": "https://www.boraflixtv.com/categoria/crime/" }, { "nome": "Documentário", "slug": "documentario", "url": "https://www.boraflixtv.com/categoria/documentario/" }, { "nome": "Drama", "slug": "drama", "url": "https://www.boraflixtv.com/categoria/drama/" }, { "nome": "Família", "slug": "familia", "url": "https://www.boraflixtv.com/categoria/familia/" }, { "nome": "Fantasia", "slug": "fantasia", "url": "https://www.boraflixtv.com/categoria/fantasia/" }, { "nome": "Faroeste", "slug": "faroeste", "url": "https://www.boraflixtv.com/categoria/faroeste/" }, { "nome": "Ficção Científica", "slug": "ficcao-cientifica", "url": "https://www.boraflixtv.com/categoria/ficcao-cientifica/" }, { "nome": "Guerra & Política", "slug": "guerra-politica", "url": "https://www.boraflixtv.com/categoria/guerra-politica/" }, { "nome": "Guerra", "slug": "guerra", "url": "https://www.boraflixtv.com/categoria/guerra/" }, { "nome": "História", "slug": "historia", "url": "https://www.boraflixtv.com/categoria/historia/" }, { "nome": "Kids", "slug": "kids", "url": "https://www.boraflixtv.com/categoria/kids/" }, { "nome": "Mistério", "slug": "misterio", "url": "https://www.boraflixtv.com/categoria/misterio/" }, { "nome": "Música", "slug": "musica", "url": "https://www.boraflixtv.com/categoria/musica/" }, { "nome": "Reality", "slug": "reality", "url": "https://www.boraflixtv.com/categoria/reality/" }, { "nome": "Romance", "slug": "romance", "url": "https://www.boraflixtv.com/categoria/romance/" }, { "nome": "Sci-Fi & Fantasy", "slug": "sci-fi-fantasy", "url": "https://www.boraflixtv.com/categoria/sci-fi-fantasy/" }, { "nome": "Soap", "slug": "soap", "url": "https://www.boraflixtv.com/categoria/soap/" }, { "nome": "Terror", "slug": "terror", "url": "https://www.boraflixtv.com/categoria/terror/" }, { "nome": "Thriller", "slug": "thriller", "url": "https://www.boraflixtv.com/categoria/thriller/" }, { "nome": "War & Politics", "slug": "war-politics", "url": "https://www.boraflixtv.com/categoria/war-politics/" }];
    const GENEROS_M03_MULTI = [{ "nome": "Ação", "slug": "acao", "url": "https://megacine.boats/generos/acao", "providerKey": "m03" }, { "nome": "Animação", "slug": "animacao", "url": "https://megacine.boats/generos/animacao", "providerKey": "m03" }, { "nome": "Aventura", "slug": "aventura", "url": "https://megacine.boats/generos/aventura", "providerKey": "m03" }, { "nome": "Comédia", "slug": "comedia", "url": "https://megacine.boats/generos/comedia", "providerKey": "m03" }, { "nome": "Crime", "slug": "crime", "url": "https://megacine.boats/generos/crime", "providerKey": "m03" }, { "nome": "Documentário", "slug": "documentario", "url": "https://megacine.boats/generos/documentario", "providerKey": "m03" }, { "nome": "Drama", "slug": "drama", "url": "https://megacine.boats/generos/drama", "providerKey": "m03" }, { "nome": "Família", "slug": "familia", "url": "https://megacine.boats/generos/familia", "providerKey": "m03" }, { "nome": "Fantasia", "slug": "fantasia", "url": "https://megacine.boats/generos/fantasia", "providerKey": "m03" }, { "nome": "Faroeste", "slug": "faroeste", "url": "https://megacine.boats/generos/faroeste", "providerKey": "m03" }, { "nome": "Ficção Científica", "slug": "ficcao-cientifica", "url": "https://megacine.boats/generos/ficcao-cientifica", "providerKey": "m03" }, { "nome": "Guerra", "slug": "guerra", "url": "https://megacine.boats/generos/guerra", "providerKey": "m03" }, { "nome": "História", "slug": "historia", "url": "https://megacine.boats/generos/historia", "providerKey": "m03" }, { "nome": "Kids", "slug": "kids", "url": "https://megacine.boats/generos/kids", "providerKey": "m03" }, { "nome": "Mistério", "slug": "misterio", "url": "https://megacine.boats/generos/misterio", "providerKey": "m03" }, { "nome": "Música", "slug": "musica", "url": "https://megacine.boats/generos/musica", "providerKey": "m03" }, { "nome": "Reality", "slug": "reality", "url": "https://megacine.boats/generos/reality", "providerKey": "m03" }, { "nome": "Romance", "slug": "romance", "url": "https://megacine.boats/generos/romance", "providerKey": "m03" }, { "nome": "Terror", "slug": "terror", "url": "https://megacine.boats/generos/terror", "providerKey": "m03" }];
    const GENEROS_E04_MULTI = [];
    const GENEROS_PRIME_MULTI = [{ "nome": "Animes", "slug": "animes", "url": "https://primeflix.mom/animes", "providerKey": "primeflix" }, { "nome": "Doramas", "slug": "doramas", "url": "https://primeflix.mom/doramas", "providerKey": "primeflix" }];
    const GENEROS_S05_MULTI = [{ "nome": "Ação", "slug": "acao", "url": "https://www.seriesonlineweb.lol/generos/acao", "providerKey": "s05" }, { "nome": "Animação", "slug": "animacao", "url": "https://www.seriesonlineweb.lol/generos/animacao", "providerKey": "s05" }, { "nome": "Aventura", "slug": "aventura", "url": "https://www.seriesonlineweb.lol/generos/aventura", "providerKey": "s05" }, { "nome": "Comédia", "slug": "comedia", "url": "https://www.seriesonlineweb.lol/generos/comedia", "providerKey": "s05" }, { "nome": "Crime", "slug": "crime", "url": "https://www.seriesonlineweb.lol/generos/crime", "providerKey": "s05" }, { "nome": "Documentário", "slug": "documentario", "url": "https://www.seriesonlineweb.lol/generos/documentario", "providerKey": "s05" }, { "nome": "Drama", "slug": "drama", "url": "https://www.seriesonlineweb.lol/generos/drama", "providerKey": "s05" }, { "nome": "Família", "slug": "familia", "url": "https://www.seriesonlineweb.lol/generos/familia", "providerKey": "s05" }, { "nome": "Fantasia", "slug": "fantasia", "url": "https://www.seriesonlineweb.lol/generos/fantasia", "providerKey": "s05" }, { "nome": "Faroeste", "slug": "faroeste", "url": "https://www.seriesonlineweb.lol/generos/faroeste", "providerKey": "s05" }, { "nome": "Ficção Científica", "slug": "ficcao-cientifica", "url": "https://www.seriesonlineweb.lol/generos/ficcao-cientifica", "providerKey": "s05" }, { "nome": "Guerra", "slug": "guerra", "url": "https://www.seriesonlineweb.lol/generos/guerra", "providerKey": "s05" }, { "nome": "História", "slug": "historia", "url": "https://www.seriesonlineweb.lol/generos/historia", "providerKey": "s05" }, { "nome": "Kids", "slug": "kids", "url": "https://www.seriesonlineweb.lol/generos/kids", "providerKey": "s05" }, { "nome": "Mistério", "slug": "misterio", "url": "https://www.seriesonlineweb.lol/generos/misterio", "providerKey": "s05" }, { "nome": "Música", "slug": "musica", "url": "https://www.seriesonlineweb.lol/generos/musica", "providerKey": "s05" }, { "nome": "Reality", "slug": "reality", "url": "https://www.seriesonlineweb.lol/generos/reality", "providerKey": "s05" }, { "nome": "Romance", "slug": "romance", "url": "https://www.seriesonlineweb.lol/generos/romance", "providerKey": "s05" }, { "nome": "Terror", "slug": "terror", "url": "https://www.seriesonlineweb.lol/generos/terror", "providerKey": "s05" }];
    const GENEROS_U06_MULTI = [{ "nome": "Ação", "slug": "acao", "url": "https://superseries.life/generos/acao", "providerKey": "u06" }, { "nome": "Animação", "slug": "animacao", "url": "https://superseries.life/generos/animacao", "providerKey": "u06" }, { "nome": "Aventura", "slug": "aventura", "url": "https://superseries.life/generos/aventura", "providerKey": "u06" }, { "nome": "Comédia", "slug": "comedia", "url": "https://superseries.life/generos/comedia", "providerKey": "u06" }, { "nome": "Crime", "slug": "crime", "url": "https://superseries.life/generos/crime", "providerKey": "u06" }, { "nome": "Documentário", "slug": "documentario", "url": "https://superseries.life/generos/documentario", "providerKey": "u06" }, { "nome": "Drama", "slug": "drama", "url": "https://superseries.life/generos/drama", "providerKey": "u06" }, { "nome": "Família", "slug": "familia", "url": "https://superseries.life/generos/familia", "providerKey": "u06" }, { "nome": "Fantasia", "slug": "fantasia", "url": "https://superseries.life/generos/fantasia", "providerKey": "u06" }, { "nome": "Faroeste", "slug": "faroeste", "url": "https://superseries.life/generos/faroeste", "providerKey": "u06" }, { "nome": "Ficção Científica", "slug": "ficcao-cientifica", "url": "https://superseries.life/generos/ficcao-cientifica", "providerKey": "u06" }, { "nome": "Guerra", "slug": "guerra", "url": "https://superseries.life/generos/guerra", "providerKey": "u06" }, { "nome": "História", "slug": "historia", "url": "https://superseries.life/generos/historia", "providerKey": "u06" }, { "nome": "Kids", "slug": "kids", "url": "https://superseries.life/generos/kids", "providerKey": "u06" }, { "nome": "Mistério", "slug": "misterio", "url": "https://superseries.life/generos/misterio", "providerKey": "u06" }, { "nome": "Música", "slug": "musica", "url": "https://superseries.life/generos/musica", "providerKey": "u06" }, { "nome": "Reality", "slug": "reality", "url": "https://superseries.life/generos/reality", "providerKey": "u06" }, { "nome": "Romance", "slug": "romance", "url": "https://superseries.life/generos/romance", "providerKey": "u06" }, { "nome": "Terror", "slug": "terror", "url": "https://superseries.life/generos/terror", "providerKey": "u06" }];
    const GENEROS_L07_MULTI = [{ "nome": "Ação", "slug": "acao", "url": "https://lisoflix.net/category/acao/", "providerKey": "l07" }, { "nome": "Action & Adventure", "slug": "action-adventure", "url": "https://lisoflix.net/category/action-adventure/", "providerKey": "l07" }, { "nome": "Animação", "slug": "animacao", "url": "https://lisoflix.net/category/animacao/", "providerKey": "l07" }, { "nome": "Aventura", "slug": "aventura", "url": "https://lisoflix.net/category/aventura/", "providerKey": "l07" }, { "nome": "Cinema TV", "slug": "cinema-tv", "url": "https://lisoflix.net/category/cinema-tv/", "providerKey": "l07" }, { "nome": "Comédia", "slug": "comedia", "url": "https://lisoflix.net/category/comedia/", "providerKey": "l07" }, { "nome": "Crime", "slug": "crime", "url": "https://lisoflix.net/category/crime/", "providerKey": "l07" }, { "nome": "Documentário", "slug": "documentario", "url": "https://lisoflix.net/category/documentario/", "providerKey": "l07" }, { "nome": "Dorama", "slug": "dorama", "url": "https://lisoflix.net/category/dorama/", "providerKey": "l07" }, { "nome": "Drama", "slug": "drama", "url": "https://lisoflix.net/category/drama/", "providerKey": "l07" }, { "nome": "Família", "slug": "familia", "url": "https://lisoflix.net/category/familia/", "providerKey": "l07" }, { "nome": "Fantasia", "slug": "fantasia", "url": "https://lisoflix.net/category/fantasia/", "providerKey": "l07" }, { "nome": "Faroeste", "slug": "faroeste", "url": "https://lisoflix.net/category/faroeste/", "providerKey": "l07" }, { "nome": "Ficção Científica", "slug": "ficcao-cientifica", "url": "https://lisoflix.net/category/ficcao-cientifica/", "providerKey": "l07" }, { "nome": "Guerra", "slug": "guerra", "url": "https://lisoflix.net/category/guerra/", "providerKey": "l07" }, { "nome": "História", "slug": "historia", "url": "https://lisoflix.net/category/historia/", "providerKey": "l07" }, { "nome": "Kids", "slug": "kids", "url": "https://lisoflix.net/category/kids/", "providerKey": "l07" }, { "nome": "Mistério", "slug": "misterio", "url": "https://lisoflix.net/category/misterio/", "providerKey": "l07" }, { "nome": "Música", "slug": "musica", "url": "https://lisoflix.net/category/musica/", "providerKey": "l07" }, { "nome": "Novelas", "slug": "novelas", "url": "https://lisoflix.net/category/novelas/", "providerKey": "l07" }, { "nome": "Reality", "slug": "reality", "url": "https://lisoflix.net/category/reality/", "providerKey": "l07" }, { "nome": "Romance", "slug": "romance", "url": "https://lisoflix.net/category/romance/", "providerKey": "l07" }, { "nome": "Sci-Fi & Fantasy", "slug": "sci-fi-fantasy", "url": "https://lisoflix.net/category/sci-fi-fantasy/", "providerKey": "l07" }, { "nome": "Soap", "slug": "soap", "url": "https://lisoflix.net/category/soap/", "providerKey": "l07" }, { "nome": "Terror", "slug": "terror", "url": "https://lisoflix.net/category/terror/", "providerKey": "l07" }, { "nome": "Thriller", "slug": "thriller", "url": "https://lisoflix.net/category/thriller/", "providerKey": "l07" }, { "nome": "Uncategorized", "slug": "uncategorized", "url": "https://lisoflix.net/category/uncategorized/", "providerKey": "l07" }];
    const ROTAS_TELA = {
        telaInicio: '/',
        telaFilmes: '/filmes/',
        telaSeries: '/series/',
    ...
    function estadoFontes() { return lerStorageObj(STORAGE_FONTES, { b01: true, p02: true, m03: true, e04: true, s05: true, u06: true, l07: true, primeflix: true }); }
    function estadoFiltros() { return lerStorageObj(STORAGE_FILTROS, { b01: true, p02: true, m03: true, e04: true, s05: true, u06: true, l07: true, primeflix: true }); }
    ...
    function providerPorUrl(url) {
        const s = String(url || '');
        if (/primeflix\.mom/i.test(s))
            return 'primeflix';
        if (/lisoflix\.net/i.test(s))
            return 'l07';
        if (/superseries\.life/i.test(s))
            return 'u06';
        if (/seriesonlineweb\.lol/i.test(s))
            return 's05';
        if (/ebaflix\.com/i.test(s))
            return 'e04';
        if (/megacine\.boats/i.test(s))
            return 'm03';
        if (/boraflixtv\.com/i.test(s))
            return 'p02';
        if (/boraflix\.click/i.test(s))
            return 'b01';
        return window.__CRONOS_RENDER_PROVIDER_KEY || 'b01';
    }
    ...
    function normalizarUrlCanonicaMulti(url) {
        ...
        if (!u)
            return '';
        try {
            u = new URL(u, PROVIDERS[providerPorUrl(u)].base).href;
        }
        catch (e) { }
        return u.replace(/[?#].*$/, '').replace(/\/+$/, '');
    }
    function providerUrl(key, urlOuPath) {
        const p = PROVIDERS[key] || PROVIDERS.b01;
        ...
        try {
            if (/^https?:\/\//i.test(raw) || raw.startsWith('//')) {
                ...
                if (hostAtual && hostAtual !== hostProvider)
                    return u.href;
    ...
    function slugGeneroMulti(titulo, urlBase) {
        try {
            const u = new URL(urlBase, PROVIDERS.b01.base);
        ...
        if (typeof slugCronos === 'function')
            return slugCronos(titulo || '');
    ...
    function urlProviderComContexto(key, urlAlvo, idTela) {
        ...
        if (eBusca && p.buscaPath) {
            let termo = '';
            try {
                termo = new URL(raw, p.base).searchParams.get('s') || '';
            }
            catch (e) {
                termo = String(raw).split('s=')[1] || '';
            }
            return new URL(p.buscaPath + encodeURIComponent(decodeURIComponent(termo)), p.base).href;
        }
        if (eAnimes)
            return key === 'primeflix' ? providerUrl(key, p.animePath || '/animes') : '';
        if (key === 'primeflix' && /\/doramas\/?/i.test(raw))
            return providerUrl(key, p.doramaPath || '/doramas');
        if (eAno && p.semAno)
            return '';
        if (eEpisodios && p.semEpisodios)
            return '';
        if (eGenero && p.semCategoria)
            return '';
        if (eFilmes && p.filmePath)
            return providerUrl(key, p.filmePath);
        if (eSeries && p.seriePath)
            return providerUrl(key, p.seriePath);
        if (eEpisodios && p.episodioPath)
            return providerUrl(key, p.episodioPath);
        if (eGenero) {
            const slug = slugGeneroMulti(tituloAtual, raw);
            if (!slug)
                return providerUrl(key, raw);
            const baseGenero = p.generoBase || '/categoria/';
            return new URL(baseGenero.replace(/\/?$/, '/') + slug + ((key === 'm03' || key === 's05') ? '' : '/'), p.base).href;
        }
        return providerUrl(key, raw);
    }
    function montarUrlPaginadaProvider(key, urlBase, pagina) {
        ...
        if (key === 'primeflix') {
            try {
                const u = new URL(base, PROVIDERS[key].base);
        ...
        try {
            ...
            if (termo !== null)
                return new URL(`/page/${pagina}/?s=${encodeURIComponent(termo)}`, PROVIDERS[key].base).href;
        }
        catch (e) { }
    ...
    function limparGrid(id) { const el = document.getElementById(id); if (el)
        el.innerHTML = ''; }
    function prepararItemParaProvider(item, key) {
        ...
        clone.querySelectorAll('img').forEach(img => {
            ['src', 'data-src', 'data-lazy-src', 'data-original', 'data-wpfc-original-src'].forEach(attr => {
                ...
                if (v)
                    img.setAttribute(attr, providerUrl(key, v));
            });
    ...
    function garantirBadgeProvider(li, key) {
        ...
        if (badgeQualidade) {
            ...
            badgeQualidade.classList.remove('provider-bfck', 'provider-bftv', 'provider-mega', 'provider-eba', 'provider-sweb', 'provider-ssup', 'provider-liso', 'provider-prime');
            badgeQualidade.classList.add(key === 'primeflix' ? 'provider-prime' : (key === 'l07' ? 'provider-liso' : (key === 'u06' ? 'provider-ssup' : (key === 's05' ? 'provider-sweb' : (key === 'e04' ? 'provider-eba' : (key === 'm03' ? 'provider-mega' : (key === 'p02' ? 'provider-bftv' : 'provider-bfck')))))));
        }
    ...
    function stampCardsNovos(grid, antes, key) {
        ...
        Array.from(grid.children).forEach(li => {
            if (!antes || !antes.has(li))
                garantirBadgeProvider(li, key || providerPorUrl(li.dataset.url));
        });
    }
    ...
    function garantirPainelProvider() {
        ...
        if (!painel) {
            ...
            if (atalho && atalho.nextSibling)
                cfg.insertBefore(painel, atalho.nextSibling);
            else {
                const titulo = cfg.querySelector('h1,h2,h3');
                if (titulo && titulo.nextSibling)
                    cfg.insertBefore(painel, titulo.nextSibling);
                else
                    cfg.insertBefore(painel, cfg.firstChild);
            }
        }
        atualizarBotoesProvider();
    }
    ...
    window.carregarConfiguracoes = function (btnElement) {
        let r;
        if (originalCarregarConfiguracoes)
            r = originalCarregarConfiguracoes.apply(this, arguments);
        else if (typeof ativarTela === 'function')
    ...
    try {
        carregarConfiguracoes = window.carregarConfiguracoes;
    }
    catch (e) { }
    ...
    function lisoAbsUrl(url) {
        ...
        if (u.startsWith('//'))
            u = 'https:' + u;
        try {
            return new URL(u, 'https://lisoflix.net/').href;
        }
        catch (e) {
            return u;
        }
    ...
    function lisoAbsImg(url) {
        ...
        if (/image\.tmdb\.org\/t\/p\//i.test(u))
            u = u.replace(/\/(w92|w154|w185|w300|w342|w500|w780|w1280|original)\//i, '/w500/');
        return u;
    }
    ...
    function lisoTipoPorUrl(url, item) {
        ...
        if (u.includes('/episodio/') || c.includes('episode'))
            return 'Episódio';
        if (u.includes('/movies/') || c.includes('movie'))
            return 'Filme';
        if (u.includes('/series/') || c.includes('tvshow') || c.includes('serie'))
            return 'Série';
        return 'Série';
    }
    function lisoPegarImagem(el) {
        ...
        for (const a of attrs) {
            ...
            if (v && !/rating|star|blank|placeholder|sprite/i.test(v))
                return v;
        }
    ...
    function extrairDadosCardLiso(item) {
        ...
        if (!url || !titulo)
            return null;
        if (!img && typeof placeholderCronosPoster === 'function')
            img = placeholderCronosPoster();
        const isMovie = tipo === 'Filme';
        return {
            id: 'l07::' + url.replace(/[?#].*$/, '').replace(/\/+$/, ''),
            url, titulo, ano, img, poster: img, backdrop: img,
            tipo, isMovie, isSerie: tipo === 'Série', qualidade: 'LISO',
            providerKey: 'l07', providerName: 'Fonte LISO', providerSigla: 'LISO', baseUrl: 'https://lisoflix.net/'
        };
    }
    function renderizarItemLisoNoGrid(item, gridId) {
        if (item && item.closest && item.closest('.MovieListSldCn, #slider-movies, #slider-tvshows, .slider, .featured, aside, .sidebar, .widget'))
            return 0;
        ...
        let tipoForcado = '';
        try {
            tipoForcado = (typeof filtroTipoGridAtual !== 'undefined' && filtroTipoGridAtual[gridId]) ? filtroTipoGridAtual[gridId] : '';
        }
        catch (e) { }
        if (!tipoForcado) {
            if (gridId === 'gridFilmes' || gridId === 'gridInicioFilmes')
                tipoForcado = 'filme';
            if (gridId === 'gridSeries' || gridId === 'gridInicioSeries')
                tipoForcado = 'serie';
            if (gridId === 'gridEpisodios' || gridId === 'gridInicioEpisodios')
                tipoForcado = 'episodio';
        }
        if (tipoForcado === 'filme' && dados.tipo !== 'Filme')
            return 0;
        if (tipoForcado === 'serie' && dados.tipo !== 'Série')
            return 0;
        if (tipoForcado === 'episodio' && dados.tipo !== 'Episódio')
            return 0;
        const canon = dados.url.replace(/[?#].*$/, '').replace(/\/+$/, '');
        const ja = Array.from(grid.querySelectorAll('[data-url]')).some(li => String(li.dataset.url || '').replace(/[?#].*$/, '').replace(/\/+$/, '') === canon && li.dataset.provider === 'l07');
        if (ja)
            return 0;
        const li = document.createElement('li');
        li.className = 'card-item ' + (dados.tipo === 'Episódio' ? 'episodio-home-card' : 'global-card');
        li.dataset.url = dados.url;
        li.dataset.provider = 'l07';
        li.dataset.providerKey = 'l07';
        ...
        li.onclick = function () {
            ...
            window.__CRONOS_RENDER_PROVIDER_KEY = 'l07';
            window.__CRONOS_LAST_PROVIDER_RENDERED = 'l07';
            try {
                analisarObra(dados.url, dados.ano || '', dados.titulo, imgFinal, dados.isMovie);
            }
            finally {
                setTimeout(() => { window.__CRONOS_RENDER_PROVIDER_KEY = prev; }, 50);
            }
        };
        grid.appendChild(li);
        try {
            if (dados.tipo !== 'Episódio' && gridId !== 'gridFavoritos' && gridId !== 'gridHistorico' && typeof adicionarBotaoFavoritarHoverCronos === 'function') {
                adicionarBotaoFavoritarHoverCronos(li, { ...dados, img: imgFinal, poster: imgFinal, providerKey: 'l07', providerName: 'Fonte LISO', providerSigla: 'LISO', baseUrl: 'https://lisoflix.net/' });
            }
        }
        catch (e) { }
        try {
            if (typeof salvarObraCronos === 'function' && dados.tipo !== 'Episódio')
    ...
    function seletorItensProvider(key, idTela) {
        if (key === 'primeflix')
            return PRIME_CARD_SELECTOR;
        if (key === 'l07')
            return LISO_ITEM_SELECTOR;
    ...
    async function renderizarItemProvider(item, gridId, key) {
        if (key === 'primeflix')
            return renderizarItemPrimeNoGrid(item, gridId);
        if (key === 'l07')
            return renderizarItemLisoNoGrid(item, gridId);
        const grid = document.getElementById(gridId);
        if (!grid)
            return 0;
        ...
        try {
            ...
            console.warn('Falha ao renderizar item do provider', key, e);
        }
        finally {
    ...
    if (originalPreencherCard) {
        ...
        try {
            preencherCardObraCronos = window.preencherCardObraCronos;
        }
        catch (e) { }
    }
    ...
    if (originalRenderEp) {
        ...
        try {
            renderizarEpisodioCronos = window.renderizarEpisodioCronos;
        }
        catch (e) { }
    }
    ...
    if (originalRenderTemp) {
        window.renderizarTemporadaCronos = function (item, gridId) {
            const grid = document.getElementById(gridId);
            const antes = grid ? new Set(Array.from(grid.children)) : null;
            const key = window.__CRONOS_RENDER_PROVIDER_KEY || 'b01';
        ...
        try {
            renderizarTemporadaCronos = window.renderizarTemporadaCronos;
        }
        catch (e) { }
    }
    ...
    async function fetchHtmlEPreencherProvider(urlAlvo, gridId, statusId, idTela, key) {
        ...
        if (!grid || !fonteAtiva(key))
            return 0;
        try {
            if (status) {
                status.innerText = `Carregando ${providerLabel(key)}...`;
                status.style.display = 'block';
                status.style.color = '';
            }
            ...
            for (const item of itens)
                total += await renderizarItemProvider(item, gridId, key);
            if (btnMais && total > 0) {
                btnMais.style.display = 'block';
                btnMais.disabled = false;
            }
            if (status)
                status.style.display = grid.children.length ? 'none' : 'block';
            aplicarFiltroVisualCronos();
            return total;
        }
        catch (err) {
            ...
            if (status) {
                status.innerText = `Erro em ${providerLabel(key)}. Continuando...`;
                status.style.display = 'block';
            }
            return 0;
        }
    ...
    async function fetchHtmlEPreencherMulti(urlAlvo, gridId, statusId, idTela) {
        ...
        for (const key of ORDEM_PROVEDORES) {
            if (!fonteAtiva(key))
                continue;
            try {
                total += await fetchHtmlEPreencherProvider(urlAlvo, gridId, statusId, idTela, key);
            }
            catch (e) {
                console.warn('Provider falhou e foi ignorado:', key, e);
            }
        }
        const status = document.getElementById(statusId);
        if (status && total === 0) {
            status.innerText = 'Nenhum resultado encontrado nas fontes ativas.';
            status.style.display = 'block';
        }
        aplicarFiltroVisualCronos();
        return total;
    ...
    try {
        fetchHtmlEPreencher = fetchHtmlEPreencherMulti;
    }
    catch (e) { }
    async function renderizarEpisodiosHomeProvider(doc, key) {
        ...
        if (!grid || !doc || !fonteAtiva(key))
            return 0;
        const seletores = key === 'l07' ? LISO_ITEM_SELECTOR : '#dt-episodes .item, .episodes .item, .episodios .item, article.episodes, article.episode';
        ...
        if (head)
            head.style.display = grid.children.length ? 'flex' : 'none';
        aplicarFiltroVisualCronos();
        return total;
    }
    ...
    function lisoExtrairImagemDetalhe(doc, modo) {
        const candidatos = [];
        const add = (v) => { if (v && !/rating|star|blank|placeholder|sprite/i.test(String(v)))
            candidatos.push(v); };
        const pegarDeEl = (el) => {
            if (!el)
                return;
            ...
            if (srcset)
                srcset.split(',').forEach(p => add(p.trim().split(/\s+/)[0]));
    ...
    function lisoExtrairBasicoDestaquePremium(item) {
        ...
        if (!url || !/lisoflix\.net/i.test(url))
            return null;
        const titleEl = item.querySelector('h3, h2, .title, .Title, .name, .Name, a[title]');
        let titulo = lisoLimparTitulo(lisoText(titleEl) ||
            linkEl.getAttribute('title') ||
            item.getAttribute('title') ||
            item.querySelector('img')?.getAttribute('alt') ||
            lisoTituloPorUrl(url));
        if (!titulo)
            titulo = lisoTituloPorUrl(url) || 'Destaque LISO';
        const candidatosImg = [];
        const add = (v) => { if (v && !/rating|star|blank|placeholder|sprite/i.test(String(v)))
            candidatosImg.push(v); };
        ...
        return {
            ...
            qualidade: 'LISO', providerKey: 'l07', providerName: 'Fonte LISO', providerSigla: 'LISO', baseUrl: 'https://lisoflix.net/'
        };
    }
    async function lisoAdicionarDestaquePremium(item, enriquecer = true) {
        ...
        if (enriquecer) {
            try {
                ...
                if (completo) {
                    dados = { ...dados, ...salvo, providerKey: 'l07', providerName: 'Fonte LISO', providerSigla: 'LISO', baseUrl: 'https://lisoflix.net/' };
                }
                else {
                    const detalhes = await lisoExtrairDetalhesDestaquePremium(dados.url);
                    dados = { ...(salvo || {}), ...dados, ...detalhes, providerKey: 'l07', providerName: 'Fonte LISO', providerSigla: 'LISO', baseUrl: 'https://lisoflix.net/' };
                }
        ...
        dados.providerKey = 'l07';
        dados.providerName = 'Fonte LISO';
        dados.providerSigla = 'LISO';
        dados.baseUrl = 'https://lisoflix.net/';
        const canon = dados.url.replace(/[?#].*$/, '').replace(/\/+$/, '');
        const ja = destaquesPremiumHome.some(o => String(o.url || '').replace(/[?#].*$/, '').replace(/\/+$/, '') === canon && o.providerKey === 'l07');
        if (!ja)
            destaquesPremiumHome.push(dados);
        return ja ? 0 : 1;
    }
    const PRIME_KEY = 'primeflix';
    ...
    function primeHtmlDecode(s) { try {
    ...
    function primeAbs(url) {
        ...
        if (u.startsWith('//'))
            u = 'https:' + u;
        try {
            return new URL(u, PRIME_BASE).href;
        }
        catch (e) {
            return u;
        }
    ...
    function primeImg(url, size) {
        let u = primeAbs(url || '');
        if (!u || /^data:image/i.test(u) || /blank|placeholder|logo|avatar|icon|rating|svg\+xml/i.test(u))
            return '';
        u = u.replace(/^https:\/\/d1muf25xaso8hp\.cloudfront\.net\/https:\/\//i, 'https://').replace(/^http:\/\//i, 'https://');
        if (/image\.tmdb\.org\/t\/p\//i.test(u))
            u = u.replace(/\/t\/p\/(w92|w154|w185|w300|w342|w500|w780|w1280|original)\//i, '/t/p/' + (size || 'w500') + '/');
        return u;
    }
    function primeTipoPorUrl(url) {
        ...
        if (u.includes('/anime/'))
            return 'Anime';
        if (u.includes('/dorama/'))
            return 'Dorama';
        if (u.includes('/episodio/'))
            return 'Episódio';
        if (u.includes('/serie/'))
            return 'Série';
        return 'Série';
    ...
    function primeTituloCard(root, link) {
        ...
        if (!t || /poster|backdrop|assista|epis[oó]dio/i.test(t))
            t = '';
        if (!t)
            t = root?.querySelector('h3[title]')?.getAttribute('title') || '';
        if (!t)
            t = primeText(root?.querySelector('h3 a, h3, h2 a, h2, .title'));
        if (!t)
            t = link?.getAttribute('title') || primeText(link);
    ...
    function primeTextosFolha(el) {
        ...
        try {
            ...
            while ((n = walker.nextNode()))
                arr.push(String(n.nodeValue || '').replace(/\s+/g, ' ').trim());
        }
        catch (e) { }
        return arr;
    }
    ...
    function primeValidoSerie(t) {
        if (!t || t.length < 3 || t.length > 90)
            return false;
        if (/^(novo|leg|dub|s[ée]rie|anime|dorama|filme|epis[oó]dio)$/i.test(t))
            return false;
        if (/^epis[oó]dio\s*\d+$/i.test(t) || /^T\s*\d+\s*:\s*E\s*\d+$/i.test(t))
            return false;
    ...
    function primeDadosEpisodio(root, url) {
        ...
        for (let i = 0; i < linhas.length; i++) {
            ...
            if (m) {
                temporada = temporada || m[1];
                episodio = episodio || m[2];
                for (let j = i + 1; j < Math.min(linhas.length, i + 5); j++) {
                    const prox = primeLimparLinhaEp(linhas[j]);
                    if (primeValidoSerie(prox)) {
                        serieTitulo = prox;
                        break;
                    }
                }
        ...
        if (imgAlt && !/^Epis[oó]dio\s*\d+$/i.test(imgAlt) && !/poster|backdrop/i.test(imgAlt))
            tituloEp = primeLimparTitulo(imgAlt);
        if (!tituloEp) {
            const linhaTitulo = linhas.map(x => primeLimparTitulo(x)).find(x => /^Epis[oó]dio\s*\d+/i.test(x));
            if (linhaTitulo)
                tituloEp = linhaTitulo;
        }
        if (!serieTitulo) {
            const candidatos = linhas.map(primeLimparLinhaEp).filter(primeValidoSerie).sort((a, b) => a.length - b.length);
            serieTitulo = candidatos[0] || '';
        }
        if (!serieTitulo) {
            let slug = String(url || '').split('/episodio/')[1] || '';
            slug = slug.split(/[?#]/)[0].replace(/-?\d+x\d+.*$/i, '').replace(/-/g, ' ').trim();
            if (slug)
                serieTitulo = slug.replace(/\b\w/g, c => c.toUpperCase());
        }
        if (!tituloEp)
            tituloEp = episodio ? `Episódio ${parseInt(episodio, 10)}` : 'Episódio';
    ...
    function primeExtrairCards(docOrRoot, filtro) {
        ...
        function aceitar(url) {
            if (!url)
                return false;
            if (filtro === 'filmes')
                return url.includes('/filme/');
            if (filtro === 'series')
                return url.includes('/serie/') || url.includes('/dorama/');
            if (filtro === 'animes')
                return url.includes('/anime/');
            if (filtro === 'episodios')
                return url.includes('/episodio/');
    ...
    function renderizarCardPrimeDados(obra, gridId) {
        const grid = document.getElementById(gridId);
        if (!grid || !obra || !obra.url || primeCardJaExiste(grid, obra.url))
            return 0;
        ...
        try {
            if (typeof salvarObraCronos === 'function' && !ehEp)
                salvarObraCronos({ ...obra, providerKey: PRIME_KEY, providerName: 'Fonte PRIME', providerSigla: 'PRIME', baseUrl: PRIME_BASE });
        }
        catch (e) { }
        aplicarFiltroVisualCronos();
        return 1;
    ...
    function renderizarItemPrimeNoGrid(item, gridId) {
        ...
        let total = 0;
        for (const obra of cards.slice(0, 1))
            total += renderizarCardPrimeDados(obra, gridId);
        return total;
    }
    ...
    function primeBackdropDetalhe(doc, html) {
        const arr = [];
        const add = v => { v = String(v || '').trim(); if (v)
            arr.push(primeHtmlDecode(v)); };
    ...
    function primeSinopseDetalhe(doc) {
        ...
        for (const sel of sels) {
            for (const el of Array.from(doc.querySelectorAll(sel))) {
                const t = primeLimparSinopse(primeText(el));
                if (t && t.length > 35 && !/compartilhar|trailer|login|cadastro/i.test(t))
                    return t;
            }
        }
    ...
    function primeGenerosDetalhe(doc, desc) {
        const set = new Set();
        const m = String(desc || '').match(/G[eê]neros:\s*([^|•]+)/i);
        if (m)
            m[1].split(',').map(x => x.trim()).filter(Boolean).forEach(g => set.add(g));
        doc.querySelectorAll('a[href*="genre"], a[href*="genero"], a[href*="/genero"], span, a').forEach(el => { const t = primeText(el); if (!t || t.length > 35)
            return; if (/Ação|Aventura|Drama|Comédia|Romance|Terror|Crime|Documentário|Família|Animação|Ficção|Faroeste|Mistério|Thriller|Fantasia/i.test(t))
            set.add(t); });
    ...
    async function primeEnriquecerDestaque(obra) {
        ...
        try {
            ...
            if (salvo && salvo.backdrop && salvo.sinopse && (salvo.poster || salvo.img))
                return { ...obra, ...salvo, providerKey: PRIME_KEY, providerName: 'Fonte PRIME', providerSigla: 'PRIME', baseUrl: PRIME_BASE };
        }
        catch (e) { }
        try {
            ...
            console.warn('Falha ao enriquecer destaque PRIME:', obra.url, e);
            return obra;
        }
    }
    async function primeAdicionarDestaques(lista) {
        ...
        for (const base of (lista || []).slice(0, 8)) {
            ...
            dados.providerKey = PRIME_KEY;
            dados.providerName = 'Fonte PRIME';
            dados.providerSigla = 'PRIME';
            dados.baseUrl = PRIME_BASE;
            ...
            if (typeof atualizarDestaquePremium === 'function')
                atualizarDestaquePremium(0);
            if (typeof esperar === 'function')
                await esperar(80);
        }
        return total;
    }
    async function carregarHomePrimeProvider(doc, status) {
        if (status)
            status.innerText = 'Carregando Home PRIME...';
        let destaques = primeExtrairHero(doc);
        if (!destaques.length)
            destaques = primeCardsDaSecao(doc, '.swiper_top10_home', '');
        if (!destaques.length)
            destaques = primeExtrairCards(doc, '').slice(0, 8);
        ...
        let cf = 0;
        for (const o of filmes.slice(0, 12))
            cf += renderizarCardPrimeDados(o, 'gridInicioFilmes');
        let cs = 0;
        for (const o of series.slice(0, 12))
            cs += renderizarCardPrimeDados(o, 'gridInicioSeries');
        let ce = 0;
        for (const o of episodios.slice(0, 12))
            ce += renderizarCardPrimeDados(o, 'gridInicioEpisodios');
        if (cf < 12)
            await completarHomeProviderAte(PRIME_KEY, '/filmes', 'gridInicioFilmes', PRIME_CARD_SELECTOR, 12);
        if (cs < 12)
            await completarHomeProviderAte(PRIME_KEY, '/series', 'gridInicioSeries', PRIME_CARD_SELECTOR, 12);
        if (ce < 12)
            await completarHomeProviderAte(PRIME_KEY, '/episodios', 'gridInicioEpisodios', PRIME_CARD_SELECTOR, 12);
        const head = document.getElementById('headEpisodiosRecentes');
        const gridEp = document.getElementById('gridInicioEpisodios');
        if (head)
            head.style.display = gridEp && gridEp.children.length ? 'flex' : 'none';
    }
    ...
    function primeAllEpisodes(html) { const m = String(html || '').match(/window\.allEpisodes\s*=\s*(\{[\s\S]*?\})\s*;/); if (!m)
        return null; try {
    ...
    function garantirModalServidoresPrime() {
        if (document.getElementById('modalServidoresPrimeCronos'))
            return;
        const css = document.createElement('style');
        ...
        modal.id = 'modalServidoresPrimeCronos';
        modal.className = 'modal-servidores-prime-cronos';
    ...
    function abrirModalServidoresPrimeCronos(titulo, tipo, dados, obraHist) {
        ...
        const lista = document.getElementById('primeCronosFontesLista');
        if (!lista)
            return;
        lista.innerHTML = '';
    ...
    function abrirPlayerPrimeCronos(titulo, url, obraHist) {
        ...
        const tit = document.getElementById('playerTitulo');
        if (tit)
            tit.textContent = titulo || 'Reproduzindo';
        ...
        if (obraHist && typeof salvarHistoricoHome === 'function') {
            try {
                salvarHistoricoHome({ ...obraHist, playerUrl: url, ultimoAcesso: new Date().toISOString(), providerKey: PRIME_KEY, providerName: 'Fonte PRIME', providerSigla: 'PRIME' });
            }
            catch (e) { }
        }
    }
    function primeApiLabelCronos(api) {
        ...
        if (a.includes('warezcdn'))
            return 'WarezCDN';
        if (a.includes('superflixapi.best'))
            return 'Superflix Best';
        if (a.includes('superflixapi.online'))
            return 'Superflix Online';
    ...
    function renderizarBotoesPrimeDiretosCronos(area, titulo, tipo, dados, obraHist) {
        if (!area || !dados || !dados.id) {
            if (area)
                area.innerHTML = '<span style="color:#ff4d4d;font-weight:bold;">Player PRIME não encontrado.</span>';
            return;
        }
        area.innerHTML = '';
    ...
    function renderizarEpisodiosPrimeDetalhe(allEpisodes, apiContentId, mainTitle) {
        ...
        container.innerHTML = '';
        const porTemp = {};
        Object.keys(allEpisodes || {}).forEach(season => { (allEpisodes[season] || []).forEach(ep => { const sid = String(ep.season || season || '1'); (porTemp[sid] || (porTemp[sid] = [])).push(ep); }); });
        ...
        const linha4 = document.getElementById('linha4Volume');
        if (linha4) {
            linha4.style.display = 'flex';
            linha4.innerHTML = `<span class="tag-meta tag-temporada">${temps.length} TEMPORADA${temps.length !== 1 ? 'S' : ''}</span><span class="tag-meta tag-episodios">${total} EPISÓDIO${total !== 1 ? 'S' : ''}</span>`;
        }
        if (!temps.length) {
            container.innerHTML = '<h3 style="color:#aaa;text-align:center;margin-top:20px;">Episódios não encontrados nessa página.</h3>';
            return;
        }
        const tabs = document.createElement('div');
        tabs.className = 'tabs-temporadas';
        const ul = document.createElement('ul');
        ul.className = 'grid-episodios';
        function render(sid) {
            Array.from(tabs.children).forEach(b => b.classList.toggle('ativo', b.dataset.sid === sid));
            ul.innerHTML = '';
            (porTemp[sid] || []).forEach((ep, idx) => {
                const s = String(ep.season || sid || '1');
                const e = String(ep.epi_num || ep.episode || idx + 1);
    ...
    async function analisarObraPrimeCorrigido(url, ano, tituloCard, img, isMovie) {
        ...
        try {
            ...
            if (status) {
                ...
                status.innerText = 'Extraindo dados do PRIME...';
                status.style.display = 'block';
                status.style.color = '';
            }
            const bloco = document.getElementById('blocoDetalhesInfo');
            if (bloco)
                bloco.style.display = 'none';
            ['areaAcaoDetalhe', 'listaEpisodios', 'linha1Original', 'linha2Ficha', 'linha3Generos', 'linha4Volume'].forEach(id => { const el = document.getElementById(id); if (el) {
                if (id === 'linha4Volume')
            ...
            const setTxt = (id, value) => { const el = document.getElementById(id); if (el)
                el.textContent = value || ''; };
            setTxt('detalheTitulo', titulo);
            const imgEl = document.getElementById('detalheImg');
            if (imgEl)
                imgEl.src = poster || (typeof placeholderCronosPoster === 'function' ? placeholderCronosPoster() : '');
            const bgEl = document.getElementById('backdropDetalhes');
            if (bgEl)
                bgEl.style.backgroundImage = backdrop ? `url('${backdrop}')` : 'none';
            const linha2 = document.getElementById('linha2Ficha');
            if (linha2) {
                linha2.innerHTML = `${nota ? `<span class="tag-meta tag-nota">IMDb: ${primeEscapeHtml(nota)}</span>` : ''}<span class="tag-meta tag-qualidade">PRIME</span>${duracao ? `<span class="tag-meta tag-duracao">${primeEscapeHtml(duracao)}</span>` : ''}${anoFinal ? `<span class="tag-meta">${primeEscapeHtml(anoFinal)}</span>` : ''}`;
            }
            const linha3 = document.getElementById('linha3Generos');
            if (linha3)
                linha3.innerHTML = generos.map(g => `<span class="tag-meta tag-genero">${primeEscapeHtml(g)}</span>`).join('');
            setTxt('detalheSinopse', sinopse);
            if (status)
                status.style.display = 'none';
            if (bloco)
                bloco.style.display = 'flex';
            if (typeof ajustarQuebraTituloDetalheCronos === 'function')
                setTimeout(ajustarQuebraTituloDetalheCronos, 50);
            ...
            if (ehFilme) {
                ...
                const s = attrs.season || m[1] || '1';
                const e = attrs.episode || m[2] || '1';
                if (area)
                    area.innerHTML = '<button class="btn-assistir" id="btnPrimePlayEpCronos">▶ Assistir Episódio</button>';
                const btn = document.getElementById('btnPrimePlayEpCronos');
                if (btn)
                    btn.onclick = () => abrirModalServidoresPrimeCronos(titulo, 'serie', { id: apiContentId, s, e }, obraSendoVista);
            }
            else {
                const all = primeAllEpisodes(html);
                if (apiContentId && all)
                    renderizarEpisodiosPrimeDetalhe(all, apiContentId, titulo);
                else if (area)
                    area.innerHTML = '<span style="color:#ff4d4d;font-weight:bold;">Episódios não encontrados nessa página.</span>';
            }
        }
        catch (err) {
            console.error(err);
            if (status) {
                ...
                status.innerText = 'Erro crítico ao processar as informações do PRIME.';
                status.style.display = 'block';
            }
        }
    }
    async function carregarHomeMulti() {
        ...
        try {
            ...
            destaquePremiumAtual = 0;
        }
        catch (e) { }
        const headEp = document.getElementById('headEpisodiosRecentes');
        if (headEp)
            headEp.style.display = 'none';
        if (!existeFonteAtiva()) {
            if (status)
                statusNenhumaFonte('statusInicio');
            return;
        }
        if (status) {
            status.style.display = 'block';
            status.style.color = '';
            status.innerText = 'Carregando fontes em ordem: ' + ORDEM_PROVEDORES.map(providerLabel).join(' → ') + '...';
        }
        for (const key of ORDEM_PROVEDORES) {
            if (!fonteAtiva(key))
                continue;
            try {
                ...
                const doc = new DOMParser().parseFromString(html, 'text/html');
                if (status)
                    status.innerText = `Carregando Home ${providerLabel(key)}...`;
                if (key === 'primeflix') {
                    await carregarHomePrimeProvider(doc, status);
                    continue;
                }
                if (key !== 'l07')
                    await renderizarEpisodiosHomeProvider(doc, key);
                const destaques = Array.from(doc.querySelectorAll(key === 'l07' ? LISO_DESTAQUE_SELECTOR : '#featured-titles .item')).slice(0, 8);
                for (let i = 0; i < destaques.length; i++) {
                    if (key === 'l07') {
                        try {
                            ...
                            console.warn('Destaque LISO falhou', e);
                        }
                        if (typeof esperar === 'function')
                            await esperar(80);
                        continue;
                    ...
                    try {
                        ...
                        console.warn('Destaque falhou', key, e);
                    }
                    finally {
                        window.__CRONOS_RENDER_PROVIDER_KEY = prev;
                    }
                ...
                if (key === 'b01') {
                    const qtdBfck = destaquesPremiumHome.filter(o => (o.providerKey || providerPorUrl(o.url)) === 'b01').length;
                    const alvoBfck = Math.min(8, destaques.length);
                    if (qtdBfck < alvoBfck) {
                        for (const itemBfck of destaques) {
                            try {
                                const cloneBfck = prepararItemParaProvider(itemBfck, 'b01');
                                const basicoBfck = extrairDadosBasicosItem(cloneBfck);
                                if (!basicoBfck || !basicoBfck.url)
                                    continue;
                                const canonBfck = normalizarUrlCanonicaMulti(basicoBfck.url, 'b01');
                                const jaBfck = destaquesPremiumHome.some(o => (o.providerKey || providerPorUrl(o.url)) === 'b01' && normalizarUrlCanonicaMulti(o.url, 'b01') === canonBfck);
                                if (jaBfck)
                                    continue;
                                let dadosBfck = dadosBasicosParaObra(basicoBfck);
                                dadosBfck.providerKey = 'b01';
                                dadosBfck.providerName = PROVIDERS.b01.nome;
                                dadosBfck.providerSigla = PROVIDERS.b01.sigla;
                                dadosBfck.baseUrl = PROVIDERS.b01.base;
                                try {
                                    const detalhesBfck = await extrairDetalhesDestaquePremium(dadosBfck.url);
                                    dadosBfck = { ...dadosBfck, ...detalhesBfck, providerKey: 'b01', providerName: PROVIDERS.b01.nome, providerSigla: PROVIDERS.b01.sigla, baseUrl: PROVIDERS.b01.base };
                                }
                                catch (e) { }
                                try {
                                    await salvarObraCronos(dadosBfck);
                                }
                                catch (e) { }
                                destaquesPremiumHome.push(dadosBfck);
                                if (destaquesPremiumHome.filter(o => (o.providerKey || providerPorUrl(o.url)) === 'b01').length >= alvoBfck)
                                    break;
                            }
                            catch (e) {
                                console.warn('Fallback destaque BFCK falhou', e);
                            }
                        }
                    }
                }
                if (key === 'l07' && !destaquesPremiumHome.some(o => o.providerKey === 'l07')) {
                    ...
                    for (const itemFb of destaquesFallback) {
                        try {
                            await lisoAdicionarDestaquePremium(itemFb, true);
                        }
                        catch (e) {
                            console.warn('Fallback destaque LISO falhou', e);
                        }
                    }
                }
                const filmes = key === 'l07' ? [] : Array.from(doc.querySelectorAll('#dt-movies .item.movies'));
                const series = key === 'l07' ? [] : Array.from(doc.querySelectorAll('#dt-tvshows .item.tvshows'));
                let cf = 0;
                for (const item of filmes) {
                    if (cf >= 12)
                        break;
                    cf += await renderizarItemProvider(item, 'gridInicioFilmes', key);
                }
                let cs = 0;
                for (const item of series) {
                    if (cs >= 12)
                        break;
                    cs += await renderizarItemProvider(item, 'gridInicioSeries', key);
                }
                if (cf < 12)
                    await completarHomeProviderAte(key, key === 'l07' ? '/movies/' : '/filmes/', 'gridInicioFilmes', key === 'l07' ? LISO_ITEM_SELECTOR : '#archive-content .item.movies, #archive-content article.item, .items .item.movies, article.item.movies', 12);
                if (cs < 12)
                    await completarHomeProviderAte(key, '/series/', 'gridInicioSeries', key === 'l07' ? LISO_ITEM_SELECTOR : '#archive-content .item.tvshows, #archive-content article.item, .items .item.tvshows, article.item.tvshows', 12);
                await completarHomeProviderAte(key, key === 'l07' ? '/episodio/' : '/episodios/', 'gridInicioEpisodios', key === 'l07' ? LISO_ITEM_SELECTOR : '#archive-content .item, #dt-episodes .item, .episodes .item, article.episodes, article.episode, .items .item', 12);
            }
            catch (e) {
        ...
        if (headEp)
            headEp.style.display = epGrid && epGrid.children.length ? 'flex' : 'none';
        try {
            destaquesPremiumHome = limitarDestaquesPremiumPorFonteCronos(destaquesPremiumHome, 8);
        }
        catch (e) { }
        if (typeof atualizarDestaquePremium === 'function')
            atualizarDestaquePremium(0);
    ...
    async function completarHomeProviderAte(key, path, gridId, seletor, limitePorFonte) {
        ...
        try {
            ...
            let total = 0;
            const seletorFinal = (key === 'l07') ? LISO_ITEM_SELECTOR : seletor;
            for (const item of doc.querySelectorAll(seletorFinal)) {
                if (count >= limitePorFonte)
                    break;
    ...
    try {
        carregarHomePage = carregarHomeMulti;
    }
    catch (e) { }
    async function iniciarNavegacaoMulti(idTela, urlAlvo, btnElement) {
        if (idTela === 'telaInicio')
            return carregarHomeMulti();
        if (idTela === 'telaCategorias') {
            if (typeof ativarTela === 'function')
                ativarTela('telaCategorias', btnElement);
            if (typeof montarCategorias === 'function')
                return montarCategorias();
        }
        if (idTela === 'telaConfiguracoes')
            return window.carregarConfiguracoes(btnElement);
        ...
        try {
            ...
            if (idTela === 'telaFilmes')
                filtroTipoGridAtual[gridId] = 'filme';
            if (idTela === 'telaSeries')
                filtroTipoGridAtual[gridId] = 'serie';
            if (idTela === 'telaEpisodios')
                filtroTipoGridAtual[gridId] = 'episodio';
            if (idTela === 'telaTemporadas')
                filtroTipoGridAtual[gridId] = 'temporada';
            if (idTela === 'telaAnimes')
                filtroTipoGridAtual[gridId] = 'anime';
        }
        catch (e) { }
        const status = document.getElementById(statusId);
        if (status) {
            status.innerText = 'Lendo fontes ativas...';
            status.style.display = 'block';
            status.style.color = '';
        }
        ...
        if (btnMais) {
            ...
            btnMais.disabled = false;
            if (typeof atualizarTextoBotaoMaisCronos === 'function')
    ...
    try {
        iniciarNavegacao = iniciarNavegacaoMulti;
    }
    catch (e) { }
    window.carregarMais = async function (idTela, urlBase) {
        ...
        for (let i = 0; i < paginasPorClique; i++) {
            for (const key of ORDEM_PROVEDORES) {
                ...
                if (status) {
                    status.innerText = `Carregando ${providerLabel(key)} página ${pag}...`;
                    status.style.display = 'block';
                    status.style.color = '';
                }
        ...
        if (status && totalItens > 0) {
            status.innerText = `Itens adicionados: ${totalItens}.`;
            status.style.display = 'block';
        }
        aplicarFiltroVisualCronos();
        return totalItens;
    ...
    try {
        carregarMais = window.carregarMais;
    }
    catch (e) { }
    window.realizarBusca = async function () {
        ...
        if (btnVoltarCategorias)
            btnVoltarCategorias.style.display = 'none';
        ...
        try {
            contextoBuscaAtual = { tipo: 'busca', baseUrl: contextoBuscaMulti.path, titulo: contextoBuscaMulti.titulo };
        }
        catch (e) { }
        ...
        if (status) {
            status.innerText = 'Pesquisando nas fontes ativas...';
            status.style.display = 'block';
            status.style.color = '';
        }
    ...
    try {
        realizarBusca = window.realizarBusca;
    }
    catch (e) { }
    function caminhoCategoriaMulti(tipo, titulo, urlBase) {
        if (tipo === 'year')
            return `/release/${String(titulo || '').trim()}/`;
        try {
            const u = new URL(urlBase, PROVIDERS.b01.base);
    ...
    window.abrirFiltroCategoria = async function (tipo, titulo, urlBase) {
        const btnVoltarCategorias = document.getElementById('btnVoltarCategorias');
        if (btnVoltarCategorias)
            btnVoltarCategorias.style.display = 'inline-flex';
        resetPaginasMulti('telaBusca');
        const grid = document.getElementById('gridBusca');
        if (grid)
            grid.innerHTML = '';
        const tituloBusca = document.getElementById('tituloBusca');
        if (tituloBusca)
            tituloBusca.innerText = tipo === 'year' ? `Lançamentos de ${titulo}` : `Categoria: ${titulo}`;
        const status = document.getElementById('statusBusca');
        if (status) {
            status.innerText = 'Consultando fontes ativas...';
            status.style.display = 'block';
            status.style.color = '';
        }
        const btnMais = document.getElementById('btnMaisBusca');
    ...
    try {
        abrirFiltroCategoria = window.abrirFiltroCategoria;
    }
    catch (e) { }
    ...
    try {
        carregarMaisFiltroAtual = window.carregarMaisFiltroAtual;
    }
    catch (e) { }
    ...
    if (originalMontarCategorias) {
            ...
            try {
                ...
                if (gridOficiais) {
                    ...
                    Array.from(gridOficiais.querySelectorAll('button.btn-cat, button')).forEach(btn => {
                        ...
                        if (nome && !mapaGeneros.has(chave))
                            mapaGeneros.set(chave, { nome, btn });
                    });
                    const extras = []
                        .concat(Array.isArray(GENEROS_P02_MULTI) ? GENEROS_P02_MULTI : [])
                        .concat(Array.isArray(GENEROS_M03_MULTI) ? GENEROS_M03_MULTI : [])
                        .concat(Array.isArray(GENEROS_E04_MULTI) ? GENEROS_E04_MULTI : [])
                        .concat(Array.isArray(GENEROS_S05_MULTI) ? GENEROS_S05_MULTI : [])
                        .concat(Array.isArray(GENEROS_U06_MULTI) ? GENEROS_U06_MULTI : [])
                        .concat(Array.isArray(GENEROS_L07_MULTI) ? GENEROS_L07_MULTI : [])
        ...
        try {
            montarCategorias = window.montarCategorias;
        }
        catch (e) { }
    }
    ...
    if (originalAnalisarObra) {
        window.analisarObra = function (url, ano, tituloCard, img, isMovie) {
            ...
            if (!fonteAtiva(key)) {
                mostrarAvisoFonteDesativada(key);
                return false;
            }
        ...
        try {
            analisarObra = window.analisarObra;
        }
        catch (e) { }
    }
    ...
    if (originalNormalizar) {
        ...
        try {
            normalizarObraParaBanco = window.normalizarObraParaBanco;
        }
        catch (e) { }
    }
    async function corrigirProviderStore(store) {
        ...
        for (const item of todos) {
            ...
            if (item.providerKey !== key || item.id !== idNovo || item.providerSigla !== PROVIDERS[key].sigla) {
                ...
                if (typeof dbDelete === 'function' && item.id && item.id !== idNovo)
                    await dbDelete(store, item.id).catch(() => { });
    ...
    if (originalSalvarHistorico) {
        ...
        try {
            salvarHistoricoHome = window.salvarHistoricoHome;
        }
        catch (e) { }
    }
    ...
    function lisoExtrairSinopseDetalhePremium(doc) {
        ...
        if (desc) {
            const paragrafos = Array.from(desc.querySelectorAll('p')).filter(p => {
                ...
                if (/Director|Genre|Cast|Cast-sh/i.test(cls))
                    return false;
                if (/^(Diretor|Gênero|Elenco)\s*:/i.test(txt))
                    return false;
            ...
            if (txtDesc && txtDesc.length > 20 && !/^(Diretor|Gênero|Elenco)/i.test(txtDesc))
                return txtDesc;
        }
    ...
    function lisoExtrairTituloDetalheCorrigido(doc, fallback, url) {
        ...
        for (const sel of seletores) {
            ...
            if (txt && txt.length > 1 && !ruim.test(txt))
                return txt;
        }
        ...
        if (metaTitulo && !ruim.test(metaTitulo))
            return metaTitulo;
    ...
    function lisoExtrairPosterDetalheCorrigido(doc, fallback) {
        ...
        const add = (v) => {
            ...
            if (!raw || lisoImagemRuimDetalheCronos(raw))
                return;
            const norm = lisoNormalizarImagemDetalheCronos(raw, 'poster');
            if (!norm || lisoImagemRuimDetalheCronos(norm))
                return;
    ...
    function lisoExtrairGenerosDetalheCorrigido(doc) {
        ...
        const addGenero = (a) => {
            ...
            if (!nome || /^(diretor|elenco|gênero|genero)$/i.test(nome))
                return;
            const href = a.getAttribute('href') || a.href || '';
            if (href && !/\/category\//i.test(href))
                return;
        ...
        areas.forEach(area => {
            ...
            if (!generos.length)
                area.querySelectorAll('.genres a[href*="/category/"], .sgeneros a[href*="/category/"]').forEach(addGenero);
        });
        return generos;
    }
    ...
    function lisoRenderizarGradeEpisodios(temporadas, tituloSerie, posterSerie, backdropSerie, anoSerie) {
        ...
        if (bTem)
            bTem.innerText = `${sids.length} TEMPORADA${sids.length !== 1 ? 'S' : ''}`;
        if (bEps)
            bEps.innerText = `${totalEps} EPISÓDIO${totalEps !== 1 ? 'S' : ''}`;
        if (!totalEps) {
        ...
        const render = (sid) => {
            ...
            lista.forEach((ep, idx) => {
                ...
                try {
                    if (typeof salvarEpisodioCronos === 'function')
    ...
    async function lisoCarregarTemporadasSerie(doc, url, titulo, poster, backdrop, ano) {
        ...
        if (Object.values(temporadas).reduce((n, arr) => n + arr.length, 0) > 0)
            return temporadas;
        ...
        for (const link of links) {
            try {
                ...
                console.warn('Falha ao buscar temporada LISO:', link, e);
            }
        }
        return temporadas;
    }
    async function analisarObraLisoCorrigido(url, ano, tituloCard, img, isMovie) {
        ...
        try {
            ...
            if (status) {
                status.className = 'loading-text';
                status.innerText = 'Extraindo metadados do Liso...';
                status.style.display = 'block';
                status.style.color = '';
            }
            ...
            ['linha1Original', 'linha2Ficha', 'linha3Generos', 'linha4Volume'].forEach(id => { const el = document.getElementById(id); if (el)
                el.innerHTML = ''; });
            const backdropEl = document.getElementById('backdropDetalhes');
            if (backdropEl)
                backdropEl.style.backgroundImage = 'none';
            obraSendoVista = { url, titulo: tituloCard, ano, img, poster: img, isMovie, providerKey: 'l07', providerName: 'Fonte LISO', providerSigla: 'LISO', baseUrl: 'https://lisoflix.net/' };
            try {
                window.obraSendoVista = obraSendoVista;
            }
            catch (e) { }
            ...
            const res = await fetch(PROXY + encodeURIComponent(url));
            if (!res.ok)
                throw new Error('Falha HTTP LISO');
            const html = await res.text();
            const doc = new DOMParser().parseFromString(html, 'text/html');
            ...
            if (iframeSrc)
                iframeSrc = lisoAbsUrl(iframeSrc.replace(/&#038;/g, '&').replace(/&amp;/g, '&'));
            ...
            if (backdropEl)
                backdropEl.style.backgroundImage = bgUrl ? `url(${bgUrl})` : 'none';
            ...
            if (duracao)
                linha2Html += `<span class="tag-meta tag-duracao">${duracao}</span>`;
            if (anoLimpo)
                linha2Html += `<span class="tag-meta">${anoLimpo}</span>`;
            ...
            obraSendoVista = { url, titulo, ano: anoLimpo || ano, img: posterSrc, poster: posterSrc, backdrop: bgUrl, sinopse, generos, isMovie: ehPlayerUnico, tipo: tipoObra, playerUrl: iframeSrc || '', providerKey: 'l07', providerName: 'Fonte LISO', providerSigla: 'LISO', baseUrl: 'https://lisoflix.net/' };
            try {
                window.obraSendoVista = obraSendoVista;
            }
            catch (e) { }
            try {
                ...
                console.warn('Falha ao salvar detalhe LISO:', e);
            }
            checarBotaoFavorito(url);
            if (status)
                status.style.display = 'none';
            ...
            if (ehPlayerUnico) {
                ...
                if (status) {
                    status.innerText = 'Carregando temporadas do Liso...';
                    status.style.display = 'block';
                }
            ...
            if (status) {
                status.className = 'error-text';
                status.innerText = 'Erro crítico ao processar as informações do Liso.';
                status.style.display = 'block';
            }
        }
    }
    ...
    if (analisarObraComMultiProviderAntesDoLiso) {
        window.analisarObra = function (url, ano, tituloCard, img, isMovie) {
            ...
            if (key === 'primeflix' || /primeflix\.mom/i.test(String(url || '')))
                return analisarObraPrimeCorrigido(url, ano, tituloCard, img, isMovie);
            if (key === 'l07' || lisoEhUrlCronos(url))
                return analisarObraLisoCorrigido(url, ano, tituloCard, img, isMovie);
